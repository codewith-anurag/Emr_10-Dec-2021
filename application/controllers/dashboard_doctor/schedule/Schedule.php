...
class Schedule extends CI_Controller {
	...
	//	);
	$session_id = $this->session->userdata('created_by');
		$d_id = $this->session->userdata('user_id');
		$isadmin =  $this->session->userdata('isadmin');
		$date = date('Y-m-d');
		   if($isadmin=='0'){
			   $where = "(doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
			$scheduless = $this->db->select("*")->from("schedule")->where($where)->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
		   }else{
			//$schedules = $this->db->select("*")->from("schedule")->where('whens',$date)->get()->result();
			$scheduless = $this->db->select("*")->from("schedule")->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
		   }
	...
	//	echo $this->session->userdata('is_admin');
		...
		if($this->session->userdata('isadmin')=='0'){
			...
			$date = date('Y-m-d');
			$where = "(whens='$date') and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
			$data['schedulestoday'] = $this->db->select("*")->from("schedule")->where($where)->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
		//	echo $this->db->last_query();
			$doctor_detail = $this->db->select("*")->from("user")->where("user_role","2")->where("user_id",$doctor_id)->where("status","1")->get()->result();
			$data['doctor'] =  $doctor_detail;
			$patient_detail = $this->db->select("*")->from("patient")->where("created_by",$doctor_id)->where("status","1")->get()->result();
			...
			$doctor_detail = $this->db->select("*")->from("user")->where("user_role","2")->where("status","1")->get()->result();
			$data['doctor'] =  $doctor_detail;
...
if(count($user)>0){
	...
	public function create()
	{
		...
		if($this->input->post('block_time_for',true)!=null){
		...
		}else if($this->input->post('doctor_id',true)!=null){
			 ...
			 if($repeat!='' and $repeat=='yes'){
 				...
 				foreach ($period as $dt) {
 				    //echo $dt->format("l Y-m-d H:i:s\n");
						...
						$arrr['hospital_id'] = $this->session->userdata('created_by');
				...
				if($inserted_id){
					if($type == "appointment"){
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"scheduleappointment","Add  Appointment",
                			$pdetail->fname." ".$pdetail->lname." Patient Appointment fixed with ".$doctordetail->firstname."  ".$doctordetail->lastname." Medical Provider at ".date('Y-m-d',strtotime($this->input->post('awhens',true)))." ".date('h:i:s',strtotime($from_time))." To ".date('h:i:s',strtotime($to_time)),$this->session->userdata("hospital_id"),$pdetail->id,$pdetail->fname." ".$pdetail->lname,10);
					}else{
						$doctordetail = $this->db->select("*")->from("user")->where("user_id",$this->input->post('block_time_for',true))->get()->row();

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"scheduleappointment","Add  Block",
                			$doctordetail->firstname."  ".$doctordetail->lastname." Medical Provider Appointment has been blocked with Patient at ".date('Y-m-d',strtotime($this->input->post('bwhens',true)))." ".date('h:i:s',strtotime($from_time))." To ".date('h:i:s',strtotime($to_time)),$this->session->userdata("hospital_id"),$doctordetail->user_id,$doctordetail->firstname." ".$doctordetail->lastname,2);						
					}
				...
				//exit;
				 	...
				 	$this->db->insert('schedule',$postData);
					//echo $this->db->last_query();
			//		exit;
					$inserted_id = $this->db->insert_id();
				 if ($inserted_id) {
					...
					if($doctordetail!='' and $pdetail!=''){
						...
						if($type == "appointment"){
							$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"scheduleappointment","Add  Appointment",
						                			$pdetail->fname." ".$pdetail->lname." Patient Appointment has been blocked with ".$doctordetail->firstname."  ".$doctordetail->lastname." Medical Provider at ".date('Y-m-d',strtotime($this->input->post('awhens',true)))." ".date('h:i:s',strtotime($from_time))." To ".date('h:i:s',strtotime($to_time)),$this->session->userdata("hospital_id"),$pdetail->id,
						                			$pdetail->fname." ".$pdetail->lname,10);
						}else{
							$doctordetail = $this->db->select("*")->from("user")->where("user_id",$this->input->post('block_time_for',true))->get()->row();
							$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"scheduleappointment","Add  Block",
						                			$doctordetail->firstname."  ".$doctordetail->lastname." Medical Provider Appointment has been blocked with Patient at ".date('Y-m-d',strtotime($this->input->post('bwhens',true)))." ".date('h:i:s',strtotime($from_time))." To ".date('h:i:s',strtotime($to_time)),$this->session->userdata("hospital_id"),$doctordetail->user_id,$doctordetail->firstname." ".$doctordetail->lastname,2);			
...
//$p_date='';
	if($type=='today'){
		...
		if($isadmin=='0'){
		$where = "Month(whens)=MONTH('$date') and YEAR(whens)=YEAR('$date') and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	}else{
	...
	if($isadmin=='0'){
		$where = "whens BETWEEN '$date' AND '$from_date' and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	}else{
		$where = "whens BETWEEN '$date' AND '$from_date'";
	}
	}
...
//$p_date='';
	if($type=='today'){
		...
		//$where = "whens BETWEEN '$date' AND '$from_date'";
		if($isadmin=='0'){
		$where = "Month(whens)=MONTH('$dates') and YEAR(whens)=YEAR('$dates') and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	}else{
...
$where = "whens BETWEEN '$date' AND '$from_date' and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	...
	$schedules = $this->db->select("*")->from("schedule")->where_in("doctor_id",$stime)->where($where)->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
	$msg ='';
	if(count($schedules)>0){
...
$where = "whens BETWEEN '$date' AND '$from_date' and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	...
	$schedules = $this->db->select("*")->from("schedule")->where_in("appointment_type",$stime)->where($where)->order_by('whens','asc')->get()->result();
	//echo $this->db->last_query();
	$msg ='';
	if(count($schedules)>0){
...
if($isadmin=='0'){
	$where = "(doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	$schedules = $this->db->select("*")->from("schedule")->where_in("appointment_type",$stime)->where($where)->where("hospital_id",$session_id)->get()->result();
}else{
	$schedules = $this->db->select("*")->from("schedule")->where_in("appointment_type",$stime)->get()->result();
}
	$i=1;
...
if($isadmin=='0'){
	...
	//$schedules = $this->db->select("*")->from("schedule")->where("doctor_id",$d_id)->get()->result();
	$schedules = $this->db->select("*")->from("schedule")->get()->result();
}
	$i=1;
	$response["events"]   = array();
	foreach ($schedules as $schedule)
...
if($isadmin=='0'){
	...
	$schedules = $this->db->select("*")->from("schedule")->where("type",'block')->get()->result();
}
	$i=1;
	$response["events"]   = array();
	foreach ($schedules as $schedule)
...
$session_id = $this->session->userdata('created_by');
if($isadmin=='0'){
$where = "Month(whens)=MONTH(CURRENT_DATE()) and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	$schedules = $this->db->select("*")->from("schedule")->where($where)->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
}else{
	$where = "Month(whens)=MONTH(CURRENT_DATE())";
		$schedules = $this->db->select("*")->from("schedule")->where($where)->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
}
	//$query = $this->db->query("select * from schedule where Month(whens) = MONTH(CURRENT_DATE())");
		//$schedules = $query->result();
...
$where = "whens BETWEEN '$this_week_sd' AND '$this_week_ed' and (doctor_id='$d_id' or block_time_for='all' or block_time_for='$d_id')";
	$schedules = $this->db->select("*")->from("schedule")->where($where)->where("hospital_id",$session_id)->order_by('whens','asc')->get()->result();
}else{
