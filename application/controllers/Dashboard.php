				...
				if ($this->form_validation->run() === true) {
						...
						if ($check_user->num_rows() === 1) {
								...
								$this->session->set_userdata([
										...
										'fullname'  => ($postData['user_role']==10)? $check_user->row()->fname.' '.$check_user->row()->lname :$check_user->row()->firstname.' '.$check_user->row()->lastname,
								...
								if($check_user->row()->is_admin==1){
									$audit_success = insert_auditdump(
										...
										"login",
										$this->session->userdata("fullname")." Login",
										$this->session->userdata("fullname")." 	User is login at ".date("Y-m-d h:i:s") ,
									 ($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
	 		 ...
	 		 $client_id = $this->input->post('client_id');
				...
				if ($this->form_validation->run() === true) {
						...
						if ($this->dashboard_model->update($postData)) {
							$get_user_info = $this->db->query("select * from user where user_id = '$user_id'")->row();
							$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"profile","Edit Profile - User",
                $get_user_info->firstname." ".$get_user_info->lastname."  ".date('d-m-Y h:i:s')." Profile is Edit  at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$get_user_info->firstname." ".$get_user_info->lastname,$this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
	...
	public function logout()
		{
			$audit_success = insert_auditdump(
										...
										$this->session->userdata("user_role"),
										"logout",
										$this->session->userdata("fullname")." Logout",
										$this->session->userdata("fullname")." 	User is logout at ".date("Y-m-d h:i:s") ,
									 ($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
