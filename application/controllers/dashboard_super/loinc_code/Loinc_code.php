...
//	$admins =	$this->lonic_model->read();
	...
	$value->create_date = date('d/m/Y',strtotime($value->create_date));
		...
		public function changestatus()
		{
			...
			$loinc_codeDetail = $this->db->select("loinc_code")->from("loinc_code")->where("loinc_code_id",$id)->get()->row();
			$arr['status'] = $value;
			$status_value = ($value == 1) ? 'Active' : 'Inactive';
			$this->db->where('loinc_code_id',$id);
			$this->db->update('loinc_code',$arr);

		 	$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"loinccode","Change Status of Loinc Code",$loinc_codeDetail->loinc_code." Loinc Code is ".$status_value."  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
			if($audit_success){
				echo 'success';
				exit;
			}
    ...
    public function email_check($email, $id)
    {
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['loinc_code_id'])) {
					if ($this->lonic_model->create($postData)) {
						...
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"loinccode","Add Loinc Code",$this->input->post('loinc_code')." Loinc Code is Add  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
		...
		redirect('dashboard_super/loinc_code/loinc_code/');
					...
					if ($this->lonic_model->update($postData)) {
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"loinccode","Update Loinc Code",$this->input->post('loinc_code')." Loinc Code is Update  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
				...
				if (!empty($_FILES['loinc_codefile']['name'])) {
			        ...
			        while($csv_line = fgetcsv($fp,1024))
			        {
			        	...
			        	if($countExistingData == 0)
	        			{
				            ...
				            $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"Loinc Code","Loinc Code  File Upload",base_url().$filename."  Loinc Code CSV File Upload  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role") );
    ...
    */
	...
	{
		$loinc_codeDetail = $this->db->select("loinc_code")->from("loinc_code")->where("loinc_code_id",$loinc_code_id)->get()->row();
		if ($this->lonic_model->delete($loinc_code_id)) {
		//	$this->delete_admin_mag($loinc_code_id);

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"loinccode","Delete Loinc Code",$loinc_codeDetail->loinc_code." Loinc Code is Delete  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
			#set success message
