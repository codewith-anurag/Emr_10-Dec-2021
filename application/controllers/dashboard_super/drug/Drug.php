...
class Drug extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_super/drug/drug_model',
	...
	{
		$data['title'] = 'Drug List';//display('admin_list');
		$isadmin = $this->session->userdata('isadmin');
					$drug_id = $this->session->userdata('drug_id');
					//if($isadmin=='1'){
				...
				//	}else{
					//	$admins =	$this->db->select("*")
							//	->from("admin")
							//	->where("created_by",$drug_id)
							//	->order_by('id','desc')
							//	->get()
							//	->result();
					//}
		$data['drug'] = $admin; //$this->drug_model->read();
		$data['content'] = $this->load->view('dashboard_super/drug/drug',$data,true);
	...
	public function sendEmailAttachment($to, $subject, $htmlMessage) {
		...
		function admindetail()
		{
			...
			$result = $this->db->select("*")->from("user")->where("drug_id",$did)->get()->row();
        ...
        $isadmin = $this->session->userdata('isadmin');
			$drug_id = $this->session->userdata('drug_id');
			 ...
			 if($p_id!=''){
				...
				// }else{
					// $sql ="SELECT * FROM admin WHERE (created_by = $drug_id) and (admin_id like '%".($p_id)."%' or fname like '%".($p_id)."%' or email like '%".($p_id)."%' or date_of_birth like '%".($p_id)."%') ORDER BY id DESC";
				// }
...
//	$admins =	$this->drug_model->read();
	...
	$objWriter->save('php://output');
		...
		public function changestatus()
		{
			...
			$drugDetail = $this->db->query("select * from drug where drug_id='$id'")->row();
			

		 $arr['status'] = $value;
		 $status_value = ($value == 1) ? "Active" : "Inactive";

		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"drug","Change Status of Drug",$drugDetail->drug_name." Drug is ".$status_value."  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));

		 $this->db->where('drug_id',$id);
		 $this->db->update('drug',$arr);
		 echo 'success';
		 exit;
		}
		public function subdrugchangestatus()
		{
			$id = $this->input->get_post('id');
			$value = $this->input->get_post('value');
    ...
    public function email_check($email, $id)
    {
		...
		{
			$data['title'] = 'Add Drug';//display('add_admin');


			#-------------------------------#

			$this->form_validation->set_rules('drug_name', 'drug','required');
			...
			if ($this->input->post('drug_id') == null) { //create a admin
				$data['drug'] = (object)$postData = [
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['drug_id'])) {
					if ($this->drug_model->create($postData)) {
						$admin_id = $this->db->insert_id();

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"drug","Add Drug",$this->input->post('drug_name')."  Drug is Add by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
					...
					redirect('dashboard_super/drug/drug/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->drug_model->update($postData)) {

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"drug","Update Drug",$this->input->post('drug_name')."  Drug is Update by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						...
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/drug/drug/edit/'.$postData['drug_id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/drug/drug_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}
		public function upload_drugname(){
		if (!empty($_FILES['drug_namefile']['name'])) {

			if (!empty($_FILES['drug_namefile']['name'])) {
				$ext = pathinfo(($_FILES['drug_namefile']['name']), PATHINFO_EXTENSION);
				$allowed = array('csv','xls');
				if (!in_array($ext, $allowed)) {
					$this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
					redirect(base_url('dashboard_super/drug/drug/upload_drugname'));
				}

				$filename  = "assets/drugfile/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['drug_namefile']['name'];
				$file_data = $_FILES['drug_namefile']['tmp_name'];
				$count=0;
        		$fp = fopen($_FILES['drug_namefile']['tmp_name'],'r') or die("can't open file");
        		move_uploaded_file($_FILES['drug_namefile']['tmp_name'], $filename);
		        while($csv_line = fgetcsv($fp,1024))
		        {
		        	$query = "SELECT * FROM drug WHERE drug_name='$csv_line[0]'";
		        	...
		        	if($countExistingData == 0)
        			{
		                ...
		                $insert_csv = array();
		                $insert_csv['drug_name'] = $csv_line[0];
		                $insert_csv['status'] = 1;
			            $data = array( 'drug_name'=>$insert_csv['drug_name'], 'status' => $insert_csv['status']);

			            $this->db->insert('drug', $data);
			            //echo $this->db->last_query();
			        }else{
			        	$this->session->set_flashdata('fail',"Drug Name Already Exists.");
						redirect(base_url('dashboard_super/drug/drug/upload_drugname'));
			        }
		        }
				...
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"Drug","Drug  File Upload",base_url().$filename." Drug CSV File Upload  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );

		       	redirect(base_url('dashboard_super/drug/drug'));
			}else{
				$this->session->set_flashdata('fail',"Drug Name File Required.");
				redirect(base_url('dashboard_super/drug/drug/upload_drugname'));
			}
		}
		$data['title'] = 'Add Drug Name';
		$data['content'] = $this->load->view('dashboard_super/drug/upload_drug',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}



		public function subdrugcreate()
		{
			$data['title'] = 'Add admin';//display('add_admin');

      $data['parent_category'] = $this->db->select("*")->from('drug_type')->where('parent_id','0')->get()->result();
			#-------------------------------#
		...
		redirect('dashboard_super/drug/drug/subcategory/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->drug_model->update($postData)) {
						#set success message
						...
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/drug/drug/edit/'.$postData['id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/drug/subdrug_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}
	...
	public function createvital()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['vital_id'])) {
				if ($this->drug_model->createvital($postData)) {
			...
			$result =	$this->db->select('*')
				...
				redirect('dashboard_super/drug/drug/profile/' . $result->id);
				//	redirect('dashboard_super/drug/drug');
			} else {
				if ($this->drug_model->updatevital($postData)) {
					#set success message
					$this->session->set_flashdata('message', display('update_successfully'));
				} else {
					#set exception message
	...
	public function profile($admin_id = null)
	{
		...
		#-------------------------------#
		$data['profile'] = $this->drug_model->read_by_id($admin_id);
	...
	//	echo $this->db->last_query();
		...
		$data['content'] = $this->load->view('dashboard_super/drug/drug_profile',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	...
	public function edit($admin_id = null)
	{
		...
		#-------------------------------#
		$data['drug'] = $this->drug_model->read_by_id($admin_id);
		$data['content'] = $this->load->view('dashboard_super/drug/drug_form',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	public function subdrugedit($admin_id = null)
	{
		$data['title'] = 'Edit';//display('admin_edit');
		#-------------------------------#
		$data['drug'] = $this->drug_model->read_by_id($admin_id);
    ...
    */
	...
	public function document()
	{
		...
		$data['documents'] = $this->document_model->read($this->session->userdata('drug_id'));
	...
	{
		$drugDetail = $this->db->query("select * from drug where drug_id='$drug_id'")->row();
			
		if ($this->drug_model->delete($drug_id)) {
		//	$this->delete_admin_mag($drug_id);

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"drug","Drug Delete",$drugDetail->drug_name." Drug is Delete  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));


			#set success message
			$this->session->set_flashdata('message','Drug Removed!');   //display('delete_successfully')
		} else {
			#set exception message
			$this->session->set_flashdata('exception',display('please_try_again'));
		}
	...
	public function subdrugdelete($drug_id = null)
	{
		if ($this->drug_model->delete($drug_id)) {
	//		$this->delete_admin_mag($drug_id);
			#set success message
	...
	public function delete_admin_mag($id)
	    {
	        $this->db->where('from_drug_id',$id);
			$this->db->delete('contact_super_admin');

			$this->db->where('to_drug_id',$id);
			$this->db->delete('contact_super_admin');
	    }
    ...
    public function document_form()
    {
        ...
        $data['document'] = (object)$postData = array(
            'admin_id'  => $this->input->post('admin_id'),
            'doctor_id'   => $this->session->userdata('drug_id'),
            'description' => $this->input->post('description'),
            'hidden_attach_file' => $this->input->post('hidden_attach_file'),
            'date'        => date('Y-m-d'),
            'upload_by'   => (($urole==10)?0:$this->session->userdata('drug_id'))
        );
        ...
        if ($this->form_validation->run() === true) {
            ...
            redirect('dashboard_super/drug/drug/document_form');
        } else {
