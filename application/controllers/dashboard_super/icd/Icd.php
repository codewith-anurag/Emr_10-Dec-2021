...
class Icd extends CI_Controller {
	...
	public function index()
	{
		...
		$isadmin = $this->session->userdata('isadmin');
					$icd_id = $this->session->userdata('icd_id');
					//if($isadmin=='1'){
				...
				//	}else{
					//	$admins =	$this->db->select("*")
							//	->from("admin")
							//	->where("created_by",$icd_id)
							//	->order_by('id','desc')
							//	->get()
							//	->result();
					//}
		$data['icd_version'] = $admin; //$this->icd_model->read();
		$data['content'] = $this->load->view('dashboard_super/icd/icd',$data,true);
		...
		$isadmin = $this->session->userdata('isadmin');
				...
				//	}else{
					//	$admins =	$this->db->select("*")
							...
							//	->result();
					//}
		$data['icd'] = $admin; //$this->icd_model->read();
		$data['content'] = $this->load->view('dashboard_super/icd/subicd',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	public function sendEmailAttachment($to, $subject, $htmlMessage) {
		...
		function admindetail()
		{
			...
			$result = $this->db->select("*")->from("user")->where("icd_id",$did)->get()->row();
        ...
        $isadmin = $this->session->userdata('isadmin');
			$icd_id = $this->session->userdata('icd_id');
			 ...
			 if($p_id!=''){
				...
				// }else{
					// $sql ="SELECT * FROM admin WHERE (created_by = $icd_id) and (admin_id like '%".($p_id)."%' or fname like '%".($p_id)."%' or email like '%".($p_id)."%' or date_of_birth like '%".($p_id)."%') ORDER BY id DESC";
				// }
...
//	$admins =	$this->icd_model->read();
	...
	$objWriter->save('php://output');
		...
		public function changestatus()
		{
			...
			$ICD_detail = $this->db->select('icd_name')->from('icd_version')->where('icd_id',$id)->get()->row();
		 $arr['status'] = $value;
		 $status_value = ($value == 1) ? 'Active' : 'Inactive';
		 $this->db->where('icd_id',$id);
		 $this->db->update('icd_version',$arr);
		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"ICD Version","Change ICD Version status",$ICD_detail->icd_name."  ICD Version status  is ". $status_value ." by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );
		 //echo $this->db->last_query();
		 echo 'success';
		 exit;
		}
		public function subicdchangestatus()
		{
			$id = $this->input->get_post('id');
			$value = $this->input->get_post('value');
    ...
    public function email_check($email, $id)
    {
		...
		{
			$data['title'] = 'Add icd';//display('add_admin');


			#-------------------------------#

			$this->form_validation->set_rules('icd_name', 'icd name','required');
			...
			if ($this->input->post('icd_id') == null) { //create a admin
				$data['icd'] = (object)$postData = [
					'icd_name'   		   => $this->input->post('icd_name'),
					'status'   		   => '1',
					'date'   		   => date('Y-m-d'),
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['icd_id'])) {
					if ($this->icd_model->create($postData)) {
						$admin_id = $this->db->insert_id();

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"ICD Version","Add ICD Version",$this->input->post('icd_name')." ICD Version is Add by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );
		...
		redirect('dashboard_super/icd/icd/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->icd_model->update($postData)) {
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"ICD Version","Update ICD Version",$this->input->post('icd_name')." ICD Version is Update by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
						$this->session->set_flashdata('message', display('update_successfully'));
					} else {
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/icd/icd/edit/'.$postData['icd_id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/icd/icd_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}

		public function upload_icdversion(){
			if (isset($_POST['save'])){

				if (!empty($_FILES['icd_codefile']['name'])) {
					$ext = pathinfo(($_FILES['icd_codefile']['name']), PATHINFO_EXTENSION);
					$allowed = array('csv','xls');
					if (!in_array($ext, $allowed)) {
						$this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
						redirect(base_url('dashboard_super/icd/icd/upload_icdversion'));
					}

					$filename  = "assets/icdversionfile/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['icd_codefile']['name'];
					$file_data = $_FILES['icd_codefile']['tmp_name'];

					$count=0;
	        		$fp = fopen($_FILES['icd_codefile']['tmp_name'],'r') or die("can't open file");
	        		move_uploaded_file($_FILES['icd_codefile']['tmp_name'], $filename);
			        while($csv_line = fgetcsv($fp,1024))
			        {
			        	$query = "SELECT * FROM icd_version WHERE icd_name='$csv_line[0]'";
			        	...
			        	if($countExistingData == 0)
	        			{
			                ...
			                $insert_csv = array();
			                $insert_csv['icd_name'] = $csv_line[0];//remove if you want to have primary key,
			                $insert_csv['status'] = 1;
				            $data = array( 'icd_name' => $insert_csv['icd_name'],  'status' => $insert_csv['status'],"date" =>date("Y-m-d"));
				            $this->db->insert('icd_version', $data);							

				        }else{
				        	$this->session->set_flashdata('fail',"ICD version Already Exists.");
							redirect(base_url('dashboard_super/icd/icd/upload_icdversion'));
				        }
			        }
					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"ICD Version","ICD Version  File Upload",base_url().$filename."  ICD Version CSV File Upload  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );
			        fclose($fp) or die("can't close file");
							$this->session->set_flashdata('message',"Import successfully.");
			       	redirect(base_url('dashboard_super/icd/icd'));
				}else{
					$this->session->set_flashdata('fail',"ICD version File Required.");
					redirect(base_url('dashboard_super/icd/icd/upload_icdversion'));
				}
			}
			$data['title'] = 'Add icd';
			$data['content'] = $this->load->view('dashboard_super/icd/icdversion_form',$data,true);
			$this->load->view('dashboard_super/main_wrapper',$data);
		}


		public function subicdcreate()
		{
			$data['title'] = 'Add admin';//display('add_admin');

      $data['parent_category'] = $this->db->select("*")->from('icd_type')->where('parent_id','0')->get()->result();
			#-------------------------------#
		...
		redirect('dashboard_super/icd/icd/subcategory/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->icd_model->update($postData)) {
						#set success message
						...
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/icd/icd/edit/'.$postData['id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/icd/subicd_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}
	...
	public function createvital()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['vital_id'])) {
				if ($this->icd_model->createvital($postData)) {
			...
			$result =	$this->db->select('*')
				...
				redirect('dashboard_super/icd/icd/profile/' . $result->id);
				//	redirect('dashboard_super/icd/icd');
			} else {
				if ($this->icd_model->updatevital($postData)) {
					#set success message
					$this->session->set_flashdata('message', display('update_successfully'));
				} else {
					#set exception message
	...
	public function profile($admin_id = null)
	{
		...
		#-------------------------------#
		$data['profile'] = $this->icd_model->read_by_id($admin_id);
	...
	//	echo $this->db->last_query();
		...
		$data['content'] = $this->load->view('dashboard_super/icd/icd_profile',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	...
	public function edit($admin_id = null)
	{
		...
		#-------------------------------#
		$data['icd'] = $this->icd_model->read_by_id($admin_id);
		$data['content'] = $this->load->view('dashboard_super/icd/icd_form',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	public function subicdedit($admin_id = null)
	{
		$data['title'] = 'Edit';//display('admin_edit');
		#-------------------------------#
		$data['icd'] = $this->icd_model->read_by_id($admin_id);
    ...
    */
	...
	public function document()
	{
		...
		$data['documents'] = $this->document_model->read($this->session->userdata('icd_id'));
	...
	{
		$ICD_detail = $this->db->select('icd_name')->from('icd_version')->where('icd_id',$icd_id)->get()->row();
		if ($this->icd_model->delete($icd_id)) {
		//	$this->delete_admin_mag($icd_id);
			#set success message
			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"ICD Version","Delete ICD Version",$ICD_detail->icd_name." ICD Version is Delete by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );
			$this->session->set_flashdata('message','icd Removed!');   //display('delete_successfully')
		} else {
			#set exception message
			$this->session->set_flashdata('exception',display('please_try_again'));
		}
	...
	public function subicddelete($icd_id = null)
	{
		if ($this->icd_model->delete($icd_id)) {
	//		$this->delete_admin_mag($icd_id);
			#set success message
	...
	public function delete_admin_mag($id)
	    {
	        $this->db->where('from_icd_id',$id);
			$this->db->delete('contact_super_admin');

			$this->db->where('to_icd_id',$id);
			$this->db->delete('contact_super_admin');
	    }
    ...
    public function document_form()
    {
        ...
        $data['document'] = (object)$postData = array(
            'admin_id'  => $this->input->post('admin_id'),
            'doctor_id'   => $this->session->userdata('icd_id'),
            'description' => $this->input->post('description'),
            'hidden_attach_file' => $this->input->post('hidden_attach_file'),
            'date'        => date('Y-m-d'),
            'upload_by'   => (($urole==10)?0:$this->session->userdata('icd_id'))
        );
        ...
        if ($this->form_validation->run() === true) {
            ...
            redirect('dashboard_super/icd/icd/document_form');
        } else {
