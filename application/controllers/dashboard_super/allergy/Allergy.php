...
class Allergy extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_super/allergy/allergy_model',
	...
	public function index()
	{
		...
		$isadmin = $this->session->userdata('isadmin');
					$allergy_id = $this->session->userdata('allergy_id');
					//if($isadmin=='1'){
				...
				//	}else{
					//	$admins =	$this->db->select("*")
							//	->from("admin")
							//	->where("created_by",$allergy_id)
							//	->order_by('id','desc')
							//	->get()
							//	->result();
					//}
		$data['allergy'] = $admin; //$this->allergy_model->read();
		$data['content'] = $this->load->view('dashboard_super/allergy/allergy',$data,true);
	...
	public function subcategory()
	{
		...
		$isadmin = $this->session->userdata('isadmin');
				...
				//	}else{
					...
					//}
		$data['allergy'] = $admin; //$this->allergy_model->read();
		$data['content'] = $this->load->view('dashboard_super/allergy/suballergy',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	public function sendEmailAttachment($to, $subject, $htmlMessage) {
		...
		function admindetail()
		{
			...
			$result = $this->db->select("*")->from("user")->where("allergy_id",$did)->get()->row();
        ...
        $isadmin = $this->session->userdata('isadmin');
			$allergy_id = $this->session->userdata('allergy_id');
			 ...
			 if($p_id!=''){
				...
				// }else{
					// $sql ="SELECT * FROM admin WHERE (created_by = $allergy_id) and (admin_id like '%".($p_id)."%' or fname like '%".($p_id)."%' or email like '%".($p_id)."%' or date_of_birth like '%".($p_id)."%') ORDER BY id DESC";
				// }
			...
			if($isadmin=='1'){
				...
				$admins =	$this->db->select("*")
						...
						->result();
	 						...
	 						//$admins =	$this->allergy_model->read();
...
//	$admins =	$this->allergy_model->read();
	...
	$value->create_date = date('d/m/Y',strtotime($value->create_date));
		...
		public function changestatus()
		{
		 ...
		 $this->db->update('allergy_type',$arr);
		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"allergy","Change Status of Allergy"," Allergy status is Change by Super Admin.",($this->session->userdata("hospital_id") !="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
		...
		public function subAllergychangestatus()
		{
			$id = $this->input->get_post('id');
			$value = $this->input->get_post('value');

		 $arr['status'] = $value;
		 $status_value = ($value == 0) ? 'Inactive' : 'Active';
		 $this->db->where('allergy_id',$id);
		 $this->db->update('allergy_type',$arr);
		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"allergy","Change Status of Allergy"," Allergy status is ". $status_value ."  by Super Admin.",($this->session->userdata("hospital_id") !="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
    ...
    public function email_check($email, $id)
    {
		...
		public function create()
		{
			...
			$this->form_validation->set_rules('allergy_name', 'Allergy name','required');
			...
			if ($this->input->post('allergy_id') == null) { //create a admin
				$data['allergy'] = (object)$postData = [
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['allergy_id'])) {
					if ($this->allergy_model->create($postData)) {
		...
		redirect('dashboard_super/allergy/allergy/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->allergy_model->update($postData)) {
						#set success message
						$this->session->set_flashdata('message', display('update_successfully'));
					} else {
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/allergy/allergy/edit/'.$postData['id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/allergy/allergy_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}

		 public function upload_allergy(){

	        if (!empty($_FILES['allergy_codefile']['name'])) {

	            if (!empty($_FILES['allergy_codefile']['name'])) {
	                $ext = pathinfo(($_FILES['allergy_codefile']['name']), PATHINFO_EXTENSION);
	                $allowed = array('csv','xls');
	                if (!in_array($ext, $allowed)) {
	                    $this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
	                    redirect(base_url('dashboard_super/allergy/allergy/upload_allergy'));
	                }

	                $filename  = "assets/allergyfile/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['allergy_codefile']['name'];
	                $file_data = $_FILES['allergy_codefile']['tmp_name'];
	                $count=0;
	                $fp = fopen($_FILES['allergy_codefile']['tmp_name'],'r') or die("can't open file");
	                move_uploaded_file($_FILES['allergy_codefile']['tmp_name'], $filename);
	                while($csv_line = fgetcsv($fp,1024))
	                {
	                    $query = "SELECT * FROM allergy_type WHERE allergy_name='$csv_line[0]'";
	                    ...
	                    if($countExistingData == 0)
	                    {
	                        ...
	                        $insert_csv = array();
	                        $insert_csv['allergy_name'] = $csv_line[0];//remove if you want to have primary key,
	                        $insert_csv['status'] = 1;
	                        $data = array( 'allergy_name'=>$insert_csv['allergy_name'], 'status' => $insert_csv['status'],"date"=>date("Y-m-d"));	                        
	                        $this->db->insert('allergy_type', $data);	                        

	                    }else{
	                        $this->session->set_flashdata('fail',"Allergy Already Exists.");
	                        redirect(base_url('dashboard_super/allergy/allergy/upload_allergy'));
	                    }
	                }
	                fclose($fp) or die("can't close file");
	                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"allergy"," Upload Allergy CSV File for Add Allergy by Super Admin",base_url().$filename." Allergy  File is Upload by Super Admin.",($this->session->userdata("hospital_id") !="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));

					$this->session->set_flashdata('message',"Import successfully.");
	                redirect(base_url('dashboard_super/allergy/allergy/subcategory'));
	            }else{
	                $this->session->set_flashdata('fail',"Allergy  File Required.");
	                redirect(base_url('dashboard_super/allergy/allergy/subcategory'));
	            }
	        }
	        $data['title'] = 'Add Allergy Code';
	        $data['content'] = $this->load->view('dashboard_super/allergy/allergy_upload',$data,true);
	        $this->load->view('dashboard_super/main_wrapper',$data);
   		}

		public function suballergycreate()
		{
			$data['title'] = 'Add admin';//display('add_admin');

      //$data['parent_category'] = $this->db->select("*")->from('allergy_type')->where('parent_id','0')->get()->result();
			#-------------------------------#
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['allergy_id'])) {
					if ($this->allergy_model->create($postData)) {
						$admin_id = $this->db->insert_id();
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"allergy","Add Allergy",$this->input->post('allergy_name')."  Allergy is Add by Super Admin.",($this->session->userdata("hospital_id") !="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
		...
		redirect('dashboard_super/allergy/allergy/subcategory/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->allergy_model->update($postData)) {
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"allergy","Edit Allergy",$this->input->post('allergy_name')."  Allergy is Edit by Super Admin.",($this->session->userdata("hospital_id") !="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
						$this->session->set_flashdata('message', display('update_successfully'));
					} else {
						#set exception message
				...
				$data['content'] = $this->load->view('dashboard_super/allergy/suballergy_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}
	...
	public function createvital()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['vital_id'])) {
				if ($this->allergy_model->createvital($postData)) {
			...
			$result =	$this->db->select('*')
				...
				redirect('dashboard_super/allergy/allergy/profile/' . $result->id);
				//	redirect('dashboard_super/allergy/allergy');
			} else {
				if ($this->allergy_model->updatevital($postData)) {
					#set success message
					$this->session->set_flashdata('message', display('update_successfully'));
				} else {
					#set exception message
	...
	public function profile($admin_id = null)
	{
		...
		#-------------------------------#
		$data['profile'] = $this->allergy_model->read_by_id($admin_id);
	...
	//	echo $this->db->last_query();
		...
		$data['content'] = $this->load->view('dashboard_super/allergy/allergy_profile',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	...
	public function edit($admin_id = null)
	{
		...
		#-------------------------------#
		$data['allergy'] = $this->allergy_model->read_by_id($admin_id);
		$data['content'] = $this->load->view('dashboard_super/allergy/allergy_form',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
	public function suballergyedit($admin_id = null)
	{
		$data['title'] = 'Edit';//display('admin_edit');
		#-------------------------------#
		$data['allergy'] = $this->allergy_model->read_by_id($admin_id);
    ...
    */
	...
	public function document()
	{
		...
		$data['documents'] = $this->document_model->read($this->session->userdata('allergy_id'));
	...
	{
		if ($this->allergy_model->delete($allergy_id)) {
		//	$this->delete_admin_mag($allergy_id);
			#set success message
			$this->session->set_flashdata('message','Allergy Removed!');   //display('delete_successfully')
		} else {
			#set exception message
			$this->session->set_flashdata('exception',display('please_try_again'));
		}
	...
	public function suballergydelete($allergy_id = null)
	{
		$allergy = $this->db->select('allergy_name')->from('allergy_type')->where('allergy_id',$allergy_id)->get()->row();
		if ($this->allergy_model->delete($allergy_id)) {


	//		$this->delete_admin_mag($allergy_id);
			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"allergy"," Delete Allergy by Super Admin",$allergy->allergy_name." Allergy  is Delete by Super Admin.",($this->session->userdata("hospital_id") !="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
			#set success message
			$this->session->set_flashdata('message','Allergy Removed!');   //display('delete_successfully')
		} else {
			#set exception message
	...
	public function delete_admin_mag($id)
	    {
	        $this->db->where('from_allergy_id',$id);
			$this->db->delete('contact_super_admin');

			$this->db->where('to_allergy_id',$id);
			$this->db->delete('contact_super_admin');
	    }
    ...
    public function document_form()
    {
        ...
        $data['document'] = (object)$postData = array(
            'admin_id'  => $this->input->post('admin_id'),
            'doctor_id'   => $this->session->userdata('allergy_id'),
            'description' => $this->input->post('description'),
            'hidden_attach_file' => $this->input->post('hidden_attach_file'),
            'date'        => date('Y-m-d'),
            'upload_by'   => (($urole==10)?0:$this->session->userdata('allergy_id'))
        );
        ...
        if ($this->form_validation->run() === true) {
            ...
            redirect('dashboard_super/allergy/allergy/document_form');
        } else {
