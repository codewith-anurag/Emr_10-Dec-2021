...
class Cvx_code extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_super/cvx_code/cvxcode_model',
	...
	{
		$data['title'] = 'CVX code List';//display('admin_list');
		$isadmin = $this->session->userdata('isadmin');
					$cvx_code_id = $this->session->userdata('cvx_code_id');
					$admin =	$this->db->select("*")
								->from("cvx_code")								
								->get()
								->result();				
		$data['cvx_code'] = $admin; 
		$data['content'] = $this->load->view('dashboard_super/cvx_code/cvx_code',$data,true);
...
$isadmin = $this->session->userdata('isadmin');
	...
	$isadmin = $this->session->userdata('isadmin');
		...
		public function changestatus()
		{
			...
			$CVXCodeDetail = $this->db->select("cvx_code")->from("cvx_code")->where("cvx_code_id",$id)->get()->row();
			$status_value = ($value == 1) ? 'Active' : 'Inactive';

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cvxcode","Change Status of CVX Code",$CVXCodeDetail->cvx_code." CVX Code is ".$status_value."  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
			

		 $arr['status'] = $value;
		 $this->db->where('cvx_code_id',$id);
		 $this->db->update('cvx_code',$arr);
		 echo 'success';
		 exit;
		}
    ...
    public function email_check($email, $id)
    {
		...
		{
			$data['title'] = 'Add CVX Code';//display('add_admin');


			#-------------------------------#

			$this->form_validation->set_rules('cvx_code', 'CVX Code','required');
			...
			if ($this->input->post('cvx_code_id') == null) { //create a admin
				$data['cvx_code'] = (object)$postData = [
					'cvx_code'   		   => $this->input->post('cvx_code'),
					'status'   		   => '1',
					'cvx_code_id'   		   => $this->input->post('cvx_code_id')
				];
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['cvx_code_id'])) {
					if ($this->cvxcode_model->create($postData)) {
						$admin_id = $this->db->insert_id();

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cvxcode","Add CVX Code",$this->input->post('cvx_code')." CVX Code is Add  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
					...
					redirect('dashboard_super/cvx_code/cvx_code/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->cvxcode_model->update($postData)) {
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cvxcode","Cvx Code Updated by  Super Admin",$this->input->post('cvx_code')." Cvx Code Updated by  Super Admin",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
						$this->session->set_flashdata('message', display('update_successfully'));
					} else {
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/cvx_code/cvx_code/edit/'.$postData['cvx_code_id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/cvx_code/cvx_code_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}

		public function upload_cvxcode(){
			
			if (!empty($_FILES['cvx_codefile']['name'])) {
				
				if (!empty($_FILES['cvx_codefile']['name'])) {
					$ext = pathinfo(($_FILES['cvx_codefile']['name']), PATHINFO_EXTENSION);
					$allowed = array('csv','xls');
					if (!in_array($ext, $allowed)) {
						$this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
						redirect(base_url('dashboard_super/cvx_code/cvx_code/upload_cvxcode'));
					}
								
					$filename  = "assets/cvxcodefile/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['cvx_codefile']['name'];
					$file_data = $_FILES['cvx_codefile']['tmp_name'];
					$count=0;
	        		$fp = fopen($_FILES['cvx_codefile']['tmp_name'],'r') or die("can't open file");
	        		move_uploaded_file($_FILES['cvx_codefile']['tmp_name'], $filename);
			        while($csv_line = fgetcsv($fp,1024))
			        {
			        	$query = "SELECT * FROM cvx_code WHERE cvx_code='$csv_line[0]'";
			        	...
			        	if($countExistingData == 0)
	        			{
			                ...
			                $insert_csv = array();
			                $insert_csv['cvx_code'] = $csv_line[0];//remove if you want to have primary key,                
			                $insert_csv['status'] = 1;
				            $data = array( 'cvx_code'=>$insert_csv['cvx_code'], 'status' => $insert_csv['status']);
				            

				            $this->db->insert('cvx_code', $data);
				           
				        }else{
				        	$this->session->set_flashdata('fail',"CVX Code Already Exists.");
							redirect(base_url('dashboard_super/cvx_code/cvx_code/upload_cvxcode'));
				        }
			        }
					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cvxcode","Upload CVX Code CSV File for Add CVX Code",base_url().$filename." CVX Code is file uplod  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
			        fclose($fp) or die("can't close file");
			       	redirect(base_url('dashboard_super/cvx_code/cvx_code'));
				}else{
					$this->session->set_flashdata('fail',"Lonic Code File Required.");
					redirect(base_url('dashboard_super/cvx_code/cvx_code/upload_cvxcode'));
				}
			}
			$data['title'] = 'Add CVX Code';
			$data['content'] = $this->load->view('dashboard_super/cvx_code/cvxcode_upload',$data,true);
			$this->load->view('dashboard_super/main_wrapper',$data);
		}
	...
	public function edit($admin_id = null)
	{
		...
		#-------------------------------#
		$data['cvx_code'] = $this->cvxcode_model->read_by_id($admin_id);
		$data['content'] = $this->load->view('dashboard_super/cvx_code/cvx_code_form',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
    ...
    */
	...
	public function delete($cvx_code_id = null)
	{
		$cvxcode_detail = $this->db->query("select * from cvx_code where cvx_code_id ='$cvx_code_id'")->row();
		
		
		if ($this->cvxcode_model->delete($cvx_code_id)) {

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cvxcode","Cvx Code Delete by  Super Admin",$cvxcode_detail->cvx_code." Cvx Code Delete by  Super Admin",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
		
			#set success message
			$this->session->set_flashdata('message','CVX code Removed!');   //display('delete_successfully')
		} else {
			#set exception message
			$this->session->set_flashdata('exception',display('please_try_again'));
		}
