...
class Manufacture extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_super/manufacture/manufacture_model',
	...
	{
		$data['title'] = 'Manufacture List';//display('admin_list');
		$isadmin = $this->session->userdata('isadmin');
					$loinc_code_id = $this->session->userdata('manufacture_id');
					//if($isadmin=='1'){
		...
		$data['manufacture'] = $admin;
		$data['content'] = $this->load->view('dashboard_super/manufacture/manufacture',$data,true);
...
//	$admins =	$this->lonic_model->read();
	...
	$objWriter->save('php://output');
		...
		public function changestatus()
		{
			...
			$manufacturedetail = $this->db->query("select * from manufacture where  manufacture_id='$id'")->row();

		 $arr['status'] = $value;
		 $status_value = ($value == 1) ? "Active" : "Inactive";
		 $this->db->where('manufacture_id',$id);
		 $this->db->update('manufacture',$arr);

		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"manufacture","Change Status of Manufacture",$manufacturedetail->manufacture." Manufacture is ".$status_value."  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
    ...
    public function email_check($email, $id)
    {
		...
		{
			$data['title'] = 'Add Manufacture';//display('add_admin');
			...
			if ($this->input->post('manufacture_id') == null) { //create a admin
				$data['manufacture'] = (object)$postData = [
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['manufacture_id'])) {
					if ($this->manufacture_model->create($postData)) {
						$admin_id = $this->db->insert_id();

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"manufacture","Add Manufacture",$this->input->post('manufacture')." Manufacture is Add by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
					...
					redirect('dashboard_super/manufacture/manufacture/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->manufacture_model->update($postData)) {

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"manufacture","Update Manufacture",$this->input->post('manufacture')." Manufacture is Update by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						...
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/manufacture/manufacture/edit/'.$postData['manufacture_id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/manufacture/manufacture_form',$data,true);
				$this->load->view('dashboard_super/main_wrapper',$data);
			}
		}

		public function upload_manufacture(){
			
			if (!empty($_FILES['manufacturefile']['name'])) {
				
				if (!empty($_FILES['manufacturefile']['name'])) {
					$ext = pathinfo(($_FILES['manufacturefile']['name']), PATHINFO_EXTENSION);
					$allowed = array('csv','xls');
					if (!in_array($ext, $allowed)) {
						$this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
						redirect(base_url('dashboard_super/manufacture/manufacture/upload_manufacture'));
					}
								
					$filename  = "assets/manufacturefile/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['manufacturefile']['name'];
					$file_data = $_FILES['manufacturefile']['tmp_name'];
					$count=0;
	        		$fp = fopen($_FILES['manufacturefile']['tmp_name'],'r') or die("can't open file");
	        		move_uploaded_file($_FILES['manufacturefile']['tmp_name'], $filename);
			        while($csv_line = fgetcsv($fp,1024))
			        {
			        	$query = "SELECT * FROM manufacture WHERE manufacture='$csv_line[0]'";
			        	...
			        	if($countExistingData == 0)
	        			{
			                ...
			                $insert_csv = array();
			                $insert_csv['manufacture'] = $csv_line[0];//remove if you want to have primary key,
			                $insert_csv['status'] = 1;
				            $data = array( 'manufacture'=>$insert_csv['manufacture'], 'status' => $insert_csv['status']);
				        	...
				        	$this->session->set_flashdata('fail',"Manufacture Already Exists.");
							redirect(base_url('dashboard_super/manufacture/manufacture/upload_manufacture'));
				        }
			        }

			        $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"manufacture","Manufacture  File Upload for Add Manufacture.",base_url().$filename."  Manufacture  CSV File Upload  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );

			        fclose($fp) or die("can't close file");
			       	redirect(base_url('dashboard_super/manufacture/manufacture'));
				}else{
					$this->session->set_flashdata('fail',"Manufacture  File Required.");
					redirect(base_url('dashboard_super/manufacture/manufacture/upload_manufacture'));
				}
			}
			$data['title'] = 'Add Lonic Code';
			$data['content'] = $this->load->view('dashboard_super/manufacture/manufacture_upload',$data,true);
			$this->load->view('dashboard_super/main_wrapper',$data);
		}
	...
	public function edit($admin_id = null)
	{
		...
		#-------------------------------#
		$data['manufacture'] = $this->manufacture_model->read_by_id($admin_id);
		$data['content'] = $this->load->view('dashboard_super/manufacture/manufacture_form',$data,true);
		$this->load->view('dashboard_super/main_wrapper',$data);
	}
    ...
    */
	...
	public function delete($manufacture_id = null)
	{
		...
		$manufacturedetail = $this->db->query("select * from manufacture where  manufacture_id='$manufacture_id'")->row();	
		if ($this->manufacture_model->delete($manufacture_id)) {
		//	$this->delete_admin_mag($loinc_code_id);

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"manufacture","Delete Manufacture",$manufacturedetail->manufacture." Manufacture is Delete by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
			#set success message
			$this->session->set_flashdata('message','Manufacture code Removed!');   //display('delete_successfully')
		} else {
			#set exception message
			$this->session->set_flashdata('exception',display('please_try_again'));
		}
