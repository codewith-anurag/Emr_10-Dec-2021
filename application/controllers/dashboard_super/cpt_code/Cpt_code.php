<?php
	...
	class Cpt_code extends CI_Controller
	{
		...
		public function __construct()
		{
			...
			$this->load->model(array(
				'dashboard_super/cpt_code/cptcode_model',
		...
		{
			$data['title'] = 'CPT code List';//display('admin_list');
			...
			$admin = $this->db->select("*")
				...
				->result();
			$data['cpt_code'] = $admin;
			$data['content'] = $this->load->view('dashboard_super/cpt_code/cpt_code', $data, true);
		...
		public function changestatus()
		{
			...
			$CPTCodeDetail = $this->db->query("select * from cpt_code where cpt_id='$id'")->row();
			$status_value = ($value == 1) ? "Active" : "Inactive";
			$arr['status'] = $value;
			$this->db->where('cpt_id', $id);
			$this->db->update('cpt_code', $arr);

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cptcode","Change Status of CPT Code",$CPTCodeDetail->cpt_code." CPT Code is ".$status_value."  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
		...
		{
			$data['title'] = 'Add CPT Code';//display('add_admin');
			$this->form_validation->set_rules('cpt_code', 'CPT Code', 'required');
			$pid = "P" . $this->randStrGen(2, 7);
			if ($this->input->post('cpt_id') == null) { //create a admin
				$data['cpt_code'] = (object)$postData = [
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['cpt_id'])) {
					if ($this->cptcode_model->create($postData)) {
						$admin_id = $this->db->insert_id();
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cptcode","Add CPT Code",$this->input->post('cpt_code')." CPT Code is Add by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
					...
					redirect('dashboard_super/cpt_code/cpt_code/');
					//redirect('admin/profile/' . $admin_id);
				} else {
					if ($this->cptcode_model->update($postData)) {

						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cptcode","Update CPT Code",$this->input->post('cpt_code')." CPT Code is Update by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
						#set success message
						$this->session->set_flashdata('message', display('update_successfully'));
					} else {
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/cpt_code/cpt_code/edit/' . $postData['cvx_code_id']);
					}
				...
				$data['content'] = $this->load->view('dashboard_super/cpt_code/cpt_code_form', $data, true);
				$this->load->view('dashboard_super/main_wrapper', $data);
			}
		}
		
		public function upload_cptcode()
		{
			
			if (!empty($_FILES['cpt_codefile']['name'])) {
				
				if (!empty($_FILES['cpt_codefile']['name'])) {
					$ext = pathinfo(($_FILES['cpt_codefile']['name']), PATHINFO_EXTENSION);
					$allowed = array('csv', 'xls');
					if (!in_array($ext, $allowed)) {
						$this->session->set_flashdata('fail', "Please Upload CSV Or xls File");
						redirect(base_url('dashboard_super/cpt_code/cpt_code/upload_cvxcode'));
					}
					
					$filename = "assets/cptcodefile/" . date('Ymdhis') . "_" . rand(999, 9999) . "_" . $_FILES['cpt_codefile']['name'];
					$file_data = $_FILES['cpt_codefile']['tmp_name'];
					$count = 0;
					$fp = fopen($_FILES['cpt_codefile']['tmp_name'], 'r') or die("can't open file");
					move_uploaded_file($_FILES['cpt_codefile']['tmp_name'], $filename);
					while ($csv_line = fgetcsv($fp, 1024)) {
						$query = "SELECT * FROM cpt_code WHERE cpt_code='$csv_line[0]'";
						...
						if ($countExistingData == 0) {
							...
							$insert_csv = array();
							$insert_csv['cpt_code'] = $csv_line[0];//remove if you want to have primary key,
							$insert_csv['status'] = 1;
							$data = array('cpt_code' => $insert_csv['cpt_code'], 'status' => $insert_csv['status']);
							
							$this->db->insert('cpt_code', $data);
							
						} else {
							$this->session->set_flashdata('fail', "CPT Code Already Exists.");
							redirect(base_url('dashboard_super/cpt_code/cpt_code/upload_cptcode'));
						}
					}
					fclose($fp) or die("can't close file");

					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cptcode","CPT Code  File Upload",base_url().$filename." CPT Code  CSV File Upload  by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role") );

					redirect(base_url('dashboard_super/cpt_code/cpt_code'));
				} else {
					$this->session->set_flashdata('fail', "CPT Code File Required.");
					redirect(base_url('dashboard_super/cpt_code/cpt_code/upload_cptcode'));
				}
			}
			$data['title'] = 'Add CPT Code';
			$data['content'] = $this->load->view('dashboard_super/cpt_code/cptcode_upload', $data, true);
			$this->load->view('dashboard_super/main_wrapper', $data);
		}
		...
		public function edit($admin_id = null)
		{
			...
			#-------------------------------#
			$data['cpt_code'] = $this->cptcode_model->read_by_id($admin_id);
			$data['content'] = $this->load->view('dashboard_super/cpt_code/cpt_code_form', $data, true);
			$this->load->view('dashboard_super/main_wrapper', $data);
		}
		...
		*/
		...
		public function delete($cpt_code_id = null)
		{
			$CPTCodeDetail = $this->db->query("select * from cpt_code where cpt_id='$cpt_code_id'")->row();

			if ($this->cptcode_model->delete($cpt_code_id)) {
				
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"cptcode","Delete Cpt Code",$CPTCodeDetail->cpt_code." CPT Code is Delete by Super Admin.",($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),$this->session->userdata("user_role"));
				#set success message
				$this->session->set_flashdata('message', 'CPT code Removed!');   //display('delete_successfully')
			} else {
				#set exception message
				$this->session->set_flashdata('exception', display('please_try_again'));
			}
