...
class Patient extends CI_Controller {
	...
	public function create_familymember($pid=""){
		...
		if (!empty($_POST)) {
			...
			if($insert_id){
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"familymember","Family Member Create",$patientdetail->fname."  ".$patientdetail->lname." Patient 's Family Member create by Admin.",$this->session->userdata("hospital_id"),$patient_id,$patientdetail->fname."  ".$patientdetail->lname,10);
	...
	public function delete_familymember($id,$patient_id){
		$patientdetail = $this->db->get_where("patient",array("id"=>$patient_id))->row();
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"familymember","Family Member Delete",$patientdetail->fname."  ".$patientdetail->lname." 's Family Member delete by Admin.",$this->session->userdata("hospital_id"),$patient_id,$patientdetail->fname."  ".$patientdetail->lname,10);
	...
	public function edit_familymember($id="")
	{
		...
		if (!empty($_POST)) {
			...
			if($sql){
				$patientdetail = $this->db->get_where("patient",array("id"=>$patient_id))->row();
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"familymember","Family Member Edit",$patientdetail->fname."  ".$patientdetail->lname." 's Family Member Edit by Admin.",$this->session->userdata("hospital_id"),$patient_id,$patientdetail->fname."  ".$patientdetail->lname,10);
				redirect(base_url().'patient/familymember/'.$patient_id);
			}
		}
		$races_result = $this->db->select("*")->from("races")->get()->result();
...
$objPHPExcel->getActiveSheet()->getStyle('M1')->applyFromArray($top_header_style);
	...
	$value->create_date = date('d/m/Y',strtotime($value->create_date));
		...
		public function changestatus()
		{
			...
			$get_patient = $this->db->query("select * from patient where patient_id='$id'")->row();
		 ...
		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"patient","Chang Status Patient",
                $get_patient->fname."  ".$get_patient->lanme." Patient  is ".$status_value." by  Admin at ".date("Y-m-d h:i:s"),($this->session->userdata("hospital_id")!="") ? $this->session->userdata("hospital_id") : $this->session->userdata("user_role") ,$get_patient->id , $get_patient->fname."  ".$get_patient->lname , 10);
			...
			if ($this->form_validation->run() === true) {
				...
				if (empty($postData['id'])) {
					if ($this->patient_model->create($postData)) {
						...
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"patient","Create Patient",$this->input->post('fname',true).' '.$this->input->post('lname',true)." Patient is Created by Admin at ".date('Y-m-d h:i:s'),$this->session->userdata("hospital_id"),$patient_id,$this->input->post('fname',true).' '.$this->input->post('lname',true),10);
					...
					if ($this->patient_model->update($postData)) {
						...
						$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"patient","Edit Patient",$this->input->post('fname',true).' '.$this->input->post('lname',true)." Patient is Edit by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_id,$this->input->post('fname',true).' '.$this->input->post('lname',true),10);
						#set success message
...
$this->dompdf->load_html($html);
	...
	{
		$get_patient = $this->patient_model->read_by_id($patient_id);
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"patient","Delete Patient",$get_patient->fname.' '.$get_patient->lname." Patient is Delete by Admin.",$this->session->userdata("hospital_id"),$patient_id,$get_patient->fname.' '.$get_patient->lname,10);
