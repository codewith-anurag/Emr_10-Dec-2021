...
class Item_master extends CI_Controller {
		...
		foreach ($searchdetail as $value) {
			$subscribe = $this->db->get_where("subscribe",array('patient_id' =>$value->id ))->result();
                ...
                foreach ($subscribe as $key => $subscribe_value) {      
				 ...
				 $msg.='<td class="text-left">'. $value->fname." ".$value->lname.'</td>';
				 $msg.='<td class="text-left">'.$total.'</td>';
				 $msg.='<td class="text-left">'.$start_date.'</td>';
				 $msg.='<td class="text-left">
				 		<a href="'.base_url("item_master/invoice/".$value->id).'" class="btn btn-xs icon-box btn-default" style="margin-right:10px;"><i class="fa fa-eye"></i></a>
				 		<a href="" class="btn btn-xs btn-danger icon-box" style="margin-right:10px;border:1px solid #f00 !important;"><i class="fa fa-trash"></i></a>
				 </td>';
				$msg.='</tr>';
	...
	public function changestatus()
	{
		...
		$itmeDetail = $this->db->query("select * from item_master where id='$id'")->row();

		$arr['status'] = $value;
		$status_value = ($value == "active") ? "Active" : "Inactive";
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemmaster","Change Status of Item",$itmeDetail->service_name." is ".$status_value."  by  Admin.", $this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$hospital_name->hospitalname,1);
	...
	public function changestatus_itemcategory()
	{
		$id = $this->input->get_post('id');
		$value = $this->input->get_post('value');
		$item_categoryDetail = $this->db->query("select * from itemcategory_master where category_id='$id'")->row();		
		$status_value = ($value == "active") ? "Active" : "Inactive";
		$arr['status'] = $value;
		...
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemcategory","Change Status of Category",$item_categoryDetail->category_name." Category is ".$status_value."  by  Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$hospital_name->hospitalname,1);
    ...
    //  exit;
			...
			if (empty($postData['id'])) {
				...
				if ($this->item_master_model->update($postData)) {
					...
					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemmaster","Update Item Master",$this->input->post('service_name',true)." Item is Updated by Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$hospital_name->hospitalname,1);
					#set success message
	...
	public function billing(){
		$data['title'] = display('bill');
		#-------------------------------#
		$data["patient"] = $this->item_master_model->read();
		$data['content']  = $this->load->view('billing' ,$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}	
	...
	public function delete_billing($patient_id)
	{
		...
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemmaster","Delete Bill",$this->input->post('service_name',true)." Item is Updated by Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$hospital_name->hospitalname,1);

		redirect(base_url('item_master/billing'));
	}
	...
	public function insert_subscribe($value='')
	{
		...
		if($this->db->insert_id()){
			if($subscribe == 0){
				...
				$userData = $this->db->select("*")->from("user")->where('user_id', $login_id)->get()->row();				
    ...
    <div class="ms-invoice-table table-responsive mt-50" style="margin-top:50px">
        ...
        $pdf_FILE = file_put_contents('pdf/'.$pdfname, $output);        
			  ...
			  /**************************   Patient Invoice Mail ************************************************************/
				...
				$subject="Invoice Payment Link ".$userData->hospitalname;
				$htmlMessage='<html>
							<head>
								<meta name="viewport" content="width=device-width" />
								<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
								<title>Payment Link  '. $userData->hospitalname .'</title>
								<style type="text/css">
									body{
				...
				$adminto = $userData->email;
				$adminsubject="Invoice Payment Link ".$userData->hospitalname;
      							...
      							<td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;">
        							<table class="table" style="border-collapse: collapse;margin-left: 15px;margin-right: 15px;margin-top:15px;width: 90%; border-bottom:1px solid #ccc; padding: 10px;" border="0">
	          							<tbody>
										 	<tr>											 												
												<td width="60%" style="border: 1px solid #d7d0d0;background-color: white; margin-left:0px; padding-left: 0px; color: #150aec;" colspan="2">									
													<p><b>Hello '.$userData->firstname.'  '.$userData->lastname.',</b></p>
												</td>
											</tr>
											<tr>		
 ...
 //echo $html;exit;
		...
		$Mail_htmlMessage = '<html>
			...
			$this->email->clear(TRUE);
					...
					$Admins_subject= $patientDetail->fname.'  '.$patientDetail->lname." Payment Done Successfully ".$userData->hospitalname;
					$AdminsMail_htmlMessage = '<html>
							<head>
								<meta name="viewport" content="width=device-width" />
								<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
								<title>Payment Done Successfully  '. $userData->hospitalname .'</title>
      							...
      							<td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;">
        							<table class="table" style="border-collapse: collapse;margin-left: 15px;margin-right: 15px;margin-top:15px;width: 90%; border-bottom:1px solid #ccc; padding: 10px;" border="0">
	          							<tbody>										 	
											...
											<tr>
													<td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;" colspan="2">
														<span>'. $patientDetail->fname.'  '.$patientDetail->lname.' Payment has been done successfully.</span>																						
													</td>
											</tr>											
											<tr>
												<td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;" colspan="2">
			...
			$this->email->subject($Admins_subject);
			$this->email->message($AdminsMail_htmlMessage);	
			$AdminFinalPDF_FILE = $_SERVER["DOCUMENT_ROOT"].'/pdf/'.$pdfname;	
			$this->email->attach($AdminFinalPDF_FILE);		
			$this->email->send();
	...
	public function update_invoice()
	{
		...
		if($this->db->affected_rows()){
			...
			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"invoice-billing","Update Invoice - Billing",$hospital_name->hospitalname." Hospital Admin ".$userData->firstname.'  '.$userData->lastname." Update Subscription of ".$patient_Details->fname." ".$patient_Details->lname ." Patient at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patientDetail->id,$patientDetail->fname." ".$patientDetail->lname,10);
			echo $this->db->insert_id();	
	...
	{
		$invoice_Detail = $this->db->get_where("subscribe",array("id"=>$invoice_id,"subscribe"=>0,"status"=>0))->row();
		$patientDetail  = $this->db->get_where("patient",array("id"=>$patient_id,"status"=>1))->row();
		$billing_date = date("d-M-Y",strtotime($invoice_Detail->start_date));
		$orderID      = $invoice_Detail->order_id;
		$amount = $invoice_Detail->amount;
		$item = explode(",", $invoice_Detail->item);
		$currency_code = 'usd';
		$login_id   = $this->session->userdata("user_id");
        ...
        $pdf_FILE = file_put_contents('pdf/'.$pdfname, $output);   
    		...
    		//print_r($result);exit;
			  ...
			  /**************************   Patient Invoice Mail ************************************************************/
				...
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"invoice-billing","Resend Invoice payment link mail of Invoice - Billing",$hospital_name->hospitalname." Hospital Admin ".$userData->firstname.'  '.$userData->lastname." Resend Invoice Payment mail link to ".$patientDetail->fname." ".$patientDetail->lname ." Patient at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patientDetail->id,$patientDetail->fname." ".$patientDetail->lname,10);
...
<div style="border:1px solid #ccc !important; padding: 25px !important; margin: 25px !important;">
 ...
 //echo $html;exit;
	...
	public function delete_subscribe($id,$patient_id)
	{
		...
		$this->db->delete('subscribe');
		if($patient_id){
			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"invoice-billing","Delete Invoice - Billing",
                $hospital_name->hospitalname." Hospital Admin ".$userData->firstname.'  '.$userData->lastname." Delete Subscription of ".$patientDetail->fname." ".$patientDetail->lname ." Patient at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patientDetail->id,$patientDetail->fname." ".$patientDetail->lname,10);
			redirect(base_url('item_master/invoice/'.$patient_id));
		}else{			
			redirect(base_url('item_master/invoice/'.$patient_id));
		}		
	...
	public function send_paymentlink_cron()
	{
		ob_start();
		$login_id   = $this->session->userdata("user_id");
		$subscribe_detail = $this->db->get_where("subscribe",array("subscribe"=>1,"status"=>0))->result();
		//print_r($subscribe_detail);
		for($i=0;$i<count($subscribe_detail);$i++){
			...
			$subscribeid = $subscribe_detail[$i]->id;
			$currency_code = 'USD';
		...
		/****************************       Mail Send To Patient Of Payment Link With Invoice *********************************************************************************/
        							...
        							<table class="table" style="border-collapse: collapse;margin-left: 15px;margin-right: 15px;margin-top:15px;width: 90%; border-bottom:1px solid #ccc; padding: 10px;" border="0">
	          							<tbody>
											...
											if($this->session->userdata("user_role") == 1){
												$htmlMessage .='<tr>
													<td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;" colspan="2">
														<p><b>Hello '.$hospital_name->hospitalname.',</b></p>																						
													</td>
												</tr>';
											}
											...
											if($this->session->userdata("user_role") == 1){
												$htmlMessage .='<tr>
													<td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;" colspan="2">
														<span> '.$patientDetail->fname."  ".$patientDetail->lname.' Payment Link has been given bellow.</span>																						
													</td>
												</tr>';
											}
				...
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"invoice-billing","Subscription Payment Link mail send - Billing",
                $hospital_name->hospitalname." Hospital Admin ".$hospital_name->firstname.'  '.$hospital_name->lastname." Add Subscription of ".$patientDetail->fname." ".$patientDetail->lname ." Patient at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patientDetail->id,$patientDetail->fname." ".$patientDetail->lname,10);				
	...
	{
		$patientDetail = $this->db->query("select * from patient where id='$patient_id'")->row();
		$this->db->where("id",$id);
		$this->db->where("patient_id",$patient_id);
		$this->db->delete("communication_log");        
		...
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"invoice-billing","Subscription Communication Log - Billing",
                $hospital_name->hospitalname." Hospital Admin ".$hospital_name->firstname.'  '.$hospital_name->lastname." Admin Delete Communication Log " .date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patientDetail->id,$patientDetail->fname." ".$patientDetail->lname,10);			

		redirect("item_master/invoice/".$patient_id);
...
// 	 }
	...
	public function add_item_category($user_id = null)
	{
		...
		if(!empty($_POST)){
			...
			if($result){
				...
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemcategory","Add Item Category",$item_category." Item Category is Add by Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),10);
	...
	public function upload_item_category($user_id = null)
	{
		$patientDetail = $this->db->query("select * from patient where id='$patient_id'")->row();
		if (!empty($_FILES['itemcategory_file']['name'])) {

			if (!empty($_FILES['itemcategory_file']['name'])) {
				$ext = pathinfo(($_FILES['itemcategory_file']['name']), PATHINFO_EXTENSION);
				$allowed = array('csv','xls');
				if (!in_array($ext, $allowed)) {
					$this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
					redirect(base_url('item_master/upload_item_category'));
				}

				$filename  = "assets/item_category_file/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['itemcategory_file']['name'];
				$file_data = $_FILES['itemcategory_file']['tmp_name'];
				$count=0;
				$fp = fopen($_FILES['itemcategory_file']['tmp_name'],'r') or die("can't open file");
				move_uploaded_file($_FILES['itemcategory_file']['tmp_name'], $filename);
				while($csv_line = fgetcsv($fp,1024))
				{
					$query = "SELECT * FROM itemcategory_master WHERE category_name='$csv_line[0]'";
					...
					if($countExistingData == 0)
					{
						...
						$insert_csv = array();
						$insert_csv['category_name'] = $csv_line[0];//remove if you want to have primary key,
						$insert_csv['status'] = "active";
						$insert_csv["hospital_id"] = $this->session->userdata("hospital_id");
						$data = array( 'category_name'=>$insert_csv['category_name'], 'status' => $insert_csv['status'],"hospital_id"=>$insert_csv['hospital_id']);

						$this->db->insert('itemcategory_master', $data);

					}else{
						$this->session->set_flashdata('fail',"Itme Category Already Exists.");
						redirect(base_url('item_master/upload_item_category'));
					}
				}

				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemmaster","Item Master Category File Upload",base_url().$filename."  Item Master Category CSV File Upload  by  Admin.", $this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$this->session->userdata("fullname"),10 );


				fclose($fp) or die("can't close file");
				$this->session->set_flashdata('message',"Import successfully.");
				redirect(base_url('item_master/item_category'));
			}else{
				$this->session->set_flashdata('fail',"Item Category File Required.");
				redirect(base_url('item_master/upload_item_category'));
			}
		}
		$data['title'] = 'Add Item Category';
		$data['content'] = $this->load->view('upload_itemcategory',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}


	public function upload_item($user_id = null)
	{
		$data['category'] = $this->item_master_model->get_category();
		$hospital_id = ($this->session->userdata('hospital_id') != "") ? $this->session->userdata('hospital_id') : $this->session->userdata('user_id');
		$userData = $this->db->select("*")->from("user")->where('user_id', $hospital_id)->get()->row();
		...
		if (!empty($_FILES['item_file']['name'])) {
			...
			if (!empty($_FILES['item_file']['name'])) {
				$ext = pathinfo(($_FILES['item_file']['name']), PATHINFO_EXTENSION);
				$allowed = array('csv','xls');
				if (!in_array($ext, $allowed)) {
					$this->session->set_flashdata('fail',"Please Upload CSV Or xls File");
					redirect(base_url('item_master/upload_item_category'));
				...
				$filename  = "assets/item_file/".date('Ymdhis')."_".rand(999,9999)."_".$_FILES['item_file']['name'];
				$file_data = $_FILES['item_file']['tmp_name'];
				$count=0;
				$fp = fopen($_FILES['item_file']['tmp_name'],'r') or die("can't open file");
				move_uploaded_file($_FILES['item_file']['tmp_name'], $filename);
				
				
				while( $csv_line = fgetcsv($fp,1024)) 
				{
					$count++;
					$query = "SELECT * FROM item_master WHERE service_name='$csv_line[0]'";
					$resultcheckExistingData = $this->db->query($query);
					$countExistingData = $resultcheckExistingData->num_rows();
					if($countExistingData == 0)
					{
						...
						$insert_csv = array();
						$insert_csv['service_name'] = $csv_line[0];//remove if you want to have primary key,
						$insert_csv['amount'] = $csv_line[2];
						$insert_csv['category'] = $this->input->post("category");
						$insert_csv['status'] = "active";
						$insert_csv['date'] = date("Y-m-d");
						$insert_csv["hospital_id"] = $this->session->userdata("hospital_id");

						$data = array( 'service_name'=>$insert_csv['service_name'], 'amount'=>$insert_csv['amount'], 'category'=>$insert_csv['category'], 'status'=>$insert_csv['status'],'date'=>$insert_csv['date'],"hospital_id"=>$insert_csv['hospital_id']);

						$this->db->insert('item_master', $data);

					}else{
						$this->session->set_flashdata('fail',"Itme  Already Exists.");
						redirect(base_url('item_master/upload_item'));
					}
					
				}
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemmaster","Item Master  File Upload",base_url().$filename."  Item Master  CSV File Upload  by  Admin.", $this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$userData->hospitalname,$this->session->userdata("user_role") );

				fclose($fp) or die("can't close file");
					$this->session->set_flashdata('message',"Import successfully.");
				   redirect(base_url('item_master'));
			}else{
				$this->session->set_flashdata('fail',"Item  File Required.");
				redirect(base_url('item_master/upload_item'));
			}
		}
		$data['title'] = 'Add Item Category';
		$data['content'] = $this->load->view('upload_item',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	...
	{
		$data['title'] = display('doctor_profile');
		$item_categories = $this->db->get_where("itemcategory_master",array("category_id"=>$id))->row();
		$hospital_id = ($this->session->userdata('hospital_id') != "") ? $this->session->userdata('hospital_id') : $this->session->userdata('user_id');
		$userData = $this->db->select("*")->from("user")->where('user_id', $hospital_id)->get()->row();
		...
		if(!empty($_POST)){
			...
			if($result){
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemcategory","Edit Item Category",$item_categories->category_name." Item Category is Edit by Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$userData->hospitalname,1);					
				redirect("item_master/item_category");
			}
		}
		
		$data["category"] = $item_categories;
	...
	{	
		...
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemcategory","Delete Item Category",$item_categoryDetail->category_name." Item Category is Delete by Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$userData->hospitalname,1);				
			...
			$query = $this->db->query($sql);
			 ...
			 //if(count($searchdetail)>0){
				 foreach ($searchdetail as $value) {
					...
					$msg.='<td class="pt-15"><div class="btn-group" style="float: right;display: flex;"><a href="'.base_url("item_master/edit_item_category/$value->category_id").'" class="btn btn-xs btn-default" style="margin-right:10px;"><i class="fa fa-edit"></i></a>';
					$msg.='<a href="'.base_url("patient/delete/$value->category_id").'" class="btn btn-xs btn-danger" onclick="return confirm('.display('are_you_sure').')"><i class="fa fa-trash"></i></a></div></td></tr>';
	...
	public function edit($item_id = null)
	{
		...
		#-------------------------------#
		$data['category'] = $this->item_master_model->get_category();
		$data['item'] = $this->item_master_model->read_by_id($item_id);
	...
	{
		$itmeDetail = $this->db->query("select * from item_master where id='$item_id'")->row();
		$hospital_id = ($this->session->userdata('hospital_id') != "") ? $this->session->userdata('hospital_id') : $this->session->userdata('user_id');
		$userData = $this->db->select("*")->from("user")->where('user_id', $login_id)->get()->row();
		if ($this->item_master_model->delete($item_id)) {

			$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"itemmaster","Delete Item",$itmeDetail->service_name." item is Delete by Admin.",$this->session->userdata("hospital_id"),$this->session->userdata("user_id"),$userData->firstname." ".$userData->lastname,1);
