...
class Billing_report extends CI_Controller {
	...
	public function index()
	{
		...
		$data['drugallergy'] = $drugallergy;   
		$data['title'] = display('Allergy Report');
		$data['patients'] = $this->patient_model->read();
		$data['content'] = $this->load->view('billing_report',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	
	function bill_report_search($patient_id=""){

		$patient_id 	= $this->input->get_post('patient_id');
		$billign_report_type 	= $this->input->get_post('billign_report_type');
		$billign_report_amount_status 	= $this->input->get_post('billign_report_amount_status');
		
		
		$sql = "";
	   		if($patient_id !="" && $billign_report_type =="" && $billign_report_amount_status == ""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.patient_id = '".$patient_id."'  ORDER BY created_date DESC";

			}else if($patient_id !="" && $billign_report_type!="" && $billign_report_amount_status==""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.patient_id = '".$patient_id."' and subscribe='".$billign_report_type."'  ORDER BY created_date DESC";

			}else if($patient_id !="" && $billign_report_amount_status!="" && $billign_report_type==""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.patient_id = '".$patient_id."' and status='".$billign_report_amount_status."'  ORDER BY created_date DESC";				
			}else if($patient_id !="" && $billign_report_type !="" && $billign_report_amount_status!=""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.patient_id = '".$patient_id."' and subscribe='".$billign_report_type."' and status='".$billign_report_amount_status."'  ORDER BY created_date DESC";
			}else if($billign_report_type !="" && $billign_report_amount_status!=""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.subscribe = '".$billign_report_type."' and status='".$billign_report_amount_status."'  ORDER BY created_date DESC";
			}else if($billign_report_type !="" && $billign_report_amount_status ==""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.subscribe = '".$billign_report_type."'   ORDER BY created_date DESC";
			}else if($billign_report_type =="" && $billign_report_amount_status !=""){
				$sql ="SELECT * FROM subscribe	WHERE  subscribe.status = '".$billign_report_amount_status."'   ORDER BY created_date DESC";
			}
			...
			$searchdetail =  $query->result();
			 ...
			 if(count($searchdetail)>0){
				 foreach ($searchdetail as $value) {
					 ...
					 $msg.='</div></div></td>';
					 $msg.='<td><span class="text-primary">'.$value->order_id.'</span></td>';
					 $msg.='<td><span class="text-primary">'.$value->amount.'</span></td>';
					 $msg.='<td><div class="kpull-left"><div class="word-break">';
					 $msg.='<span class="text-primary">'.$value->subscribe.'</span>';
					 $msg.='</div></div></td>';
					 $msg.='<td>'.$value->duration.'</td>';					 
	...
	public function download_excel()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Bill Report');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'Subscribe');
		$objPHPExcel->getActiveSheet()->setCellValue('E2', 'Subscribe Type');		
		...
		$billign_report_type 			= !empty($_GET['billign_report_type']) ? $_GET['billign_report_type'] : "";
		$billign_report_amount_status 	= !empty($_GET['billign_report_amount_status']) ? $_GET['billign_report_amount_status'] : "";
		...
		if(count($searchdetail)>0){
			foreach ($searchdetail as $value) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->amount);
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $value->subscribe);
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $value->duration = $value->duration);				
...
redirect('patient');
	...
	function billing_report_pdf()
	{
		...
		$pdfname = "Bill_report" . date('YmdHis') . '.pdf';
		$html = '<style>
								table {
					...
					$searchdetail =  $query->result();
						...
						if(count($searchdetail)>0){
							...
							$html.='</div></div></td>';
							$html.='<td><span class="text-primary">'.$value->order_id.'</span></td>';
							$html.='<td><span class="text-primary">'.$value->amount.'</span></td>';
							$html.='<td><div class="kpull-left"><div class="word-break">';
							$html.='<span class="text-primary">'.$value->subscribe.'</span>';
							$html.='</div></div></td>';
