...
$data['medical_provider'] = $this->db->select("*")->from("user")->where(user_role,2)->where(status,1)->where("hospital_id",$this->session->userdata('hospital_id'))->get()->result();
$data['patient'] = $this->db->select("*")->from("patient")->where(status,1)->where("hospital_id",$this->session->userdata('hospital_id'))->get()->result();

$module = $this->db->select("module")->from("audit_dump")->where("hospital_id",$hospital_id)->group_by("module")->get()->result();

$data['module'] = $module;
//$data['auditdump'] =  $this->db->query("SELECT * FROM (  SELECT * FROM audit_dump   ORDER BY date DESC) AS sub where role!=11 ORDER BY date DESC ")->result();
//$this->db->select("*,Date(date) as audit_date")->from("audit_dump")->where("hospital_id",$hospital_id)->order_by('audit_date','DESC')->get()->result();;
//echo $this->db->last_query();exit;
$data['auditdump'] =  $this->db->query("SELECT * FROM audit_dump where hospital_id=".$hospital_id." ORDER BY date DESC ")->result();
//echo $this->db->last_query();
//exit;
$data['title'] = display('Audit Dump');

$data['content'] = $this->load->view('auditdump',$data,true);
...
function auditdump_report_search($patient_id=""){
	...
	$sql = "";
	if($auditdump_report_from_datetime !="" && $auditdump_report_to_datetime !="" && $auditdump_report_module != "" &&  $auditdump_report_medicalprovider!="" && $auditdump_report_patient!=""){
		$sql ="SELECT *,Date(date) as audit_date FROM audit_dump	WHERE hospital_id='".$hospital_id."' and  audit_dump.date >= '".$auditdump_report_from_datetime."' and audit_dump.date <= '".$auditdump_report_to_datetime."' and module='".$auditdump_report_module."' and user='".$auditdump_report_medicalprovider."' ORDER BY audit_date DESC";

	}else if($auditdump_report_from_datetime !="" && $auditdump_report_to_datetime !="" && $auditdump_report_module == "" && $auditdump_report_medicalprovider ==""){
	...
	}else if($auditdump_report_from_datetime !="" && $auditdump_report_to_datetime !="" && $auditdump_report_medicalprovider != "" && $auditdump_report_module ==""){
		$sql ="SELECT *,Date(date) as audit_date FROM audit_dump	WHERE hospital_id='".$hospital_id."' and  audit_dump.date >= '".$auditdump_report_from_datetime."' and audit_dump.date <= '".$auditdump_report_to_datetime."' and user='".$auditdump_report_medicalprovider."' ORDER BY audit_date DESC";
	}
...
public function download_excel()
{
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Perfomming');
	$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Perfomed');
	...
	if($auditdump_report_from_datetime !="" && $auditdump_report_to_datetime !="" && $auditdump_report_module != ""){
		$sql ="SELECT *,Date(date) as audit_date FROM audit_dump	WHERE hospital_id='".$hospital_id."' and  audit_dump.date >= '".$auditdump_report_from_datetime."' and audit_dump.date <= '".$auditdump_report_to_datetime."' and module='".$auditdump_report_module."' ORDER BY audit_date DESC";

	}else if($auditdump_report_from_datetime !="" && $auditdump_report_to_datetime !="" && $auditdump_report_module == ""){
		$sql ="SELECT *,Date(date) as audit_date FROM audit_dump	WHERE hospital_id='".$hospital_id."' and  audit_dump.date >= '".$auditdump_report_from_datetime."' and audit_dump.date <= '".$auditdump_report_to_datetime."' ORDER BY audit_date DESC";
	}else if ($auditdump_report_from_datetime =="" && $auditdump_report_to_datetime =="" && $auditdump_report_module == "") {
		$sql ="SELECT * FROM audit_dump	WHERE hospital_id='".$hospital_id."'  ORDER BY date DESC";
	}
	...
	if(count($searchdetail)>0){
		foreach ($searchdetail as $value) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, $userdetail->firstname.' '.$userdetail->lastname.' ( '.$value->role.' ) ');
			if($value->performed_role == "10"){
				...
				(empty($pateintdetail)) ? $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->performed_user. $value->performed_role) : $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $pateintdetail->fname.' '.$pateintdetail->lname.  $value->performed_role);
				...
				(empty($medical_detail)) ? $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->performed_user. $value->performed_role)  : $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $medical_detail->firstname.' '.$medical_detail->lastname. $value->performed_role);
			}

			if($value->performed_role == "11"){
				$sqlQ3 = "Select firstname,lastname  from user where user_id=".$value->performed_id." and user_role=".$value->performed_role;
				$SAquery = $this->db->query($sqlQ3);
				$superdetail =  $SAquery->row();

				$value->performed_role =  ' (  '."Super Admin".' ) ';
				(empty($superdetail)) ? $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->performed_user. $value->performed_role) : $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $superdetail->firstname.' '.$superdetail->lastname.  $value->performed_role);
				...
				$Aquery = $this->db->query($sqlQ4);
				$admin_detail =  $Aquery->row();

				$value->performed_role =  ' (  '."Admin".' ) ';
				(empty($medical_detail)) ? $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->performed_user. $value->performed_role)  : $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $admin_detail->firstname.' '.$admin_detail->lastname. $value->performed_role);
			}
