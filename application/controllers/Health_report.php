...
class Health_report extends CI_Controller {
	...
	public function index()
	{
		...
		$data['content'] = $this->load->view('healthreport',$data,true);
    ...
    public function audit_patient_detail($patient_ID)
    {
        $sql = " SELECT * FROM patient WHERE id = '$patient_ID'";
        $query = $this->db->query($sql);
        $get_patient_info =  $query->row();
	...
	foreach ($patient_list as $value) {
		...
		$msg .= '<tr class="hovertr" style="border-bottom:1px solid #ddd ;">
									...
									<td>'.date('Y-m-d H:i', strtotime($value->onset_datetime)).'</td>
                                    <td>'.$value->status.'</td>
	...
	public function remove_problem()	{
      ...
      $get_patient_info = $this->audit_patient_detail($patient_id);
      $problem_detail = $this->db->query("select * from healthreport_problem where id='$patient_name'")->row();

	  $this->db->where('id',$patient_name);
	  $this->db->delete('healthreport_problem');


      $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-problemlist","Delete Problem List - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." 's  ".$problem_detail->problem_id."  ".date('d-m-Y h:i:s',strtotime($problem_detail->date))  ."  problem of Health Report  - Problem List is Delete by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function remove_allergies()	{
      //Allergy List
            ...
            $get_patient_info = $this->audit_patient_detail($patient_id);
            $allergies_detail = $this->db->query("select * from healthreport_allergies where id='$patient_name'")->row();

            $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-allergiesist","Delete Allergy List - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." 's  ".$allergies_detail->drug_allergy." : ".$allergies_detail->allergy_type.date('d-m-Y h:i:s',strtotime($allergies_detail->date))  ."  allergy of Health Report  - Allergy List is Delete by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
$html = '<title>'.$pdfname.'</title><style>
								table {
...
//$role = $this->db->select("*")->from("role")->where("r_id",$doctor->role_id)->get()->row();
								$html .='<tr>
										<td width="10%" style="padding: 5px;">'.$patient->problem_id.'</td><td style="padding: 5px;">'.$patient->icd_version_id.'</td>
...
//$role = $this->db->select("*")->from("role")->where("r_id",$doctor->role_id)->get()->row();
								$html .='<tr>
										<td width="10%" style="padding: 5px;">'.$patient->allergy_type.'</td><td style="padding: 5px;">'.$patient->drug_allergy.'</td>
...
//$role = $this->db->select("*")->from("role")->where("r_id",$doctor->role_id)->get()->row();
								$html .='<tr>
										<td width="10%" style="padding: 5px;">'.$patient->cpt_code.'</td><td style="padding: 5px;">'.$patient->description.'</td>
...
$html = '<title>'.$pdfname.'</title><style>
						...
						<h3 style="text-align:center;">Health Report For E-prescription/Medication</h3>
						<p>Patient Name :'.$patient_name->fname.' '.$patient_name->lname.'</p>
						<table border="1" width="100%" class="pricedetail" style="margin-top: 1px;">
								 <tr>
										<th>Drug Name</th>
						...
						//$patients =	$this->patient_model->read();
								...
								$html .='<tr>
										<td width="10%" style="padding: 5px;">'.$patient->drug_name.'</td><td style="padding: 5px;">'.$patient->sig.'</td>
...
public function remove_vital()	{
        ...
        $id = $this->input->post("patient_name");
		$patient_id = $this->input->post("patient_id");

        $get_patient_info = $this->audit_patient_detail($patient_id);

        $vital_datail = $this->db->query("select * from pa_vital_sign where id='$id'")->row();


	    $this->db->where('id',$id);
		$this->db->delete('pa_vital_sign');

        $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vitalsing","Delete Vital Sign - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname."  Patient 's ".date("d-m-Y h:i:s",strtotime($vital_datail->date))."  Vital sign is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function update_vital($p_id=0,$p_date=0)	{
	...
	if($patient_name!="0"){
	  $detail = $this->db->select("*")->from("pa_vital_sign")->where("id",$p_id)->where("patient_id",$patient_name)->get()->row();
        ...
        $this->session->set_userdata("edit_healthrecord_ids",$detail->healthrecord_id);
        //echo $this->session->userdata("edit_healthrecord_ids");exit;
		$problem_data =	$this->get_update_problem($detail->patient_id,$detail->date);
		  ...
		  if($p_id!="" && $p_id!= 0 && $p_date!="" && $p_date!= 0){
			  ...
			  $detail = $query->row();
               if(!empty($detail)){
                ...
                if(!empty($detail)){
    				$detail->diagnosis_datetime = date('Y-m-d h:i:s',strtotime($detail->diagnosis_datetime));
    				$detail->onset_datetime = date('Y-m-d h:i:s',strtotime($detail->onset_datetime));
    				$detail->changed_datetime = date('Y-m-d h:i:s',strtotime($detail->changed_datetime));
    				return $detail;
                }
			}
	  		//echo json_encode($detail);
    ...
    public function get_update_eprescription($p_id=0,$p_date=0)
    {
        ...
        if($p_id!="" && $p_id!= 0 && $p_date!="" && $p_date!= 0){
           ...
           $sql = "SELECT * FROM `e_prescription` WHERE `patient_id` = '$p_id' AND date(`date`) = '$p_date' ORDER BY `date` DESC";
            ...
            $detail = $query->row();
            if(!empty($detail)){
                $detail->datetime_prescribed = ($detail->datetime_prescribed!="") ? date('Y-m-d h:i:s A',strtotime($detail->datetime_prescribed)) : "";
                $detail->datetime_started_taking = ($detail->datetime_started_taking !="") ? date('Y-m-d h:i:s A',strtotime($detail->datetime_started_taking)) : "";

                $detail->datetime_stopped_taking = ($detail->datetime_stopped_taking!="") ? date('Y-m-d h:i:s A',strtotime($detail->datetime_stopped_taking)) : "";
                return $detail;
            }
        }else{
            $detail = $this->db->select("*")->from("e_prescription")->where("patient_id",$patient_name)->order_by('date','DESC')->get()->row();
            if (!empty($detail)) {
                $detail->datetime_prescribed = ($detail->datetime_prescribed!="") ? date('Y-m-d h:i:s',strtotime($detail->datetime_prescribed)) : "";
                $detail->datetime_started_taking = ($detail->datetime_started_taking !="") ? date('Y-m-d h:i:s',strtotime($detail->datetime_started_taking)) : "";

                $detail->datetime_stopped_taking = ($detail->datetime_stopped_taking!="") ? date('Y-m-d h:i:s',strtotime($detail->datetime_stopped_taking)) : "";
                return $detail;
            }
...
public function get_update_recordvaccinations($p_id=0,$p_date=0)
{
    ...
    if($p_id!="" && $p_id!= 0 && $p_date!="" && $p_date!= 0){
        ...
        if (!empty($detail)) {
            ...
            $detail->administred_on = ($detail->administred_on!="") ? date('Y-m-d h:i:s A',strtotime($detail->administred_on)) : "";
            return $detail;
        }
...
public function remove_labresult()	{
    ...
    $get_patient_info = $this->audit_patient_detail($patient_id);
    $labresult_data = $this->db->query("select  * from lab_result where  loinc_code_id='$patient_name'")->row();

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-labresult","Delete Lab Result - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." 's  ".$labresult_data->loinc_code." Loinc Code  Lab Result ".date("d-m-Y h:i:s",strtotime($labresult_data->date))."  Lab Result  is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_imaging_order()	{
    ...
    $get_patient_info = $this->audit_patient_detail($patient_id);
    $imagingorder_data = $this->db->query("select  * from imaging_order where  imaging_order_id='$patient_name'")->row();

	$this->db->where('imaging_order_id',$patient_name);
	$this->db->delete('imaging_order');

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-imagingorder","Delete Imaging order - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." 's  Patient". $imagingorder_data->cpt_code."  Imaging order ".date("d-m-Y h:i:s",strtotime($imagingorder_data->date))."  Imaging order  is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_prescription_medication()	{
    ...
    $get_patient_info = $this->audit_patient_detail($patient_id);
    $prescription_medication_data = $this->db->query("select  * from e_prescription where  e_prescription_id='$patient_name'")->row();

	$this->db->where('e_prescription_id',$patient_name);
	$this->db->delete('e_prescription');

     $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),
        "healthreport-E-prescription/medication","Delete E-prescription/Medication - Health Report",
         $get_patient_info->fname." ".$get_patient_info->lname." 's Patient ". $imagingorder_data->cpt_code. $prescription_medication_data->drug_name."  E-prescription/Medication ".date("d-m-Y h:i:s",strtotime($prescription_medication_data->date))."  E-prescription/Medication is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_Vaccines()    {
    ...
    $query = $this->db->query("select * from healthreport_vaccines where patient_id='$patient_id' and id='$patient_name'");
    $no  = $query->num_rows();
    $Vaccines_DATA = $query->row();


    $this->db->where('id',$patient_name);
    $this->db->delete('healthreport_vaccines');

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vitalsing","Delete Vaccines - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".date("d-m-Y h:i:s",strtotime($Vaccines_DATA->date))."  Vaccines  is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_recordvaccinations()	{
    ...
    $get_patient_info = $this->audit_patient_detail($patient_id);
    $recordvaccinations_DATA = $this->db->query("select * from healthreport_recordvaccinations where id='$patient_name'")->row();

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-recordvaccinations","Delete Record Vaccinations - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname."  Patient ".date("d-m-Y h:i:s",strtotime($recordvaccinations_DATA->date))."  Record Vaccinations is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	//	$status = ($patient_list->status==1)?'Active':'Inactive';
	  //<td> Dr.'.$doctor->firstname.' '.$doctor->lastname.'</td>
                              ...
                              <td>
                                ...
                                <div style="float: right;" class="btn-group">
                                  <a target="_blank" href="'.base_url().'health_report/all_report_pdf/'.$value->healthrecord_id.'" class="btn btn-xs icon-box" ><i class="fa fa-print"></i></a>
                                  <a href="javascript:void(0);" class="btn btn-xs icon-box" onClick="update_vital(' . $value->id . ','.$p_id.','.$p_date.')"><i class="fa fa-edit"></i></a>
...
foreach ($patient_list as $value) {
    ...
    $reviewsing_data   = $this->db->get_where("healthreport_reviewsign",array("id"=>$patient_name))->row();

	$this->db->where('id',$patient_name);
	$this->db->delete('healthreport_reviewsign');
    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-reviewsing","Delete Review Sign - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname." 's Patient ".date("d-m-Y h:i:s",strtotime($reviewsing_data->date))."  Review Sign is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_summary()	{
    ...
    $get_patient_info = $this->audit_patient_detail($patient_id);
    $summary_DATA = $this->db->query("select * from  healthreport_summary where id = '$patient_name' ")->row();

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-summary","Delete Summary - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname." 's Patient ".date("d-m-Y h:i:s",strtotime($summary_DATA->date))."  Summary is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_uploadeddocument()	{
    ...
    $get_patient_info = $this->audit_patient_detail($patient_id);
    $uploaddoc_DATA = $this->db->query("select * from healthreport_document where id='$patient_name'")->row();

    $doc_type = "";

    if($uploaddoc_DATA->document_type == 1){
        $doc_type = "Locked Clinical Notes";
    }elseif ($uploaddoc_DATA->document_type == 2) {
        $doc_type = "Signed Consent Forms";
    }elseif ($uploaddoc_DATA->document_type == 3) {
        $doc_type = "Lab Result";
    }elseif ($uploaddoc_DATA->document_type == 4) {
        $doc_type = "Amendments";
    }
    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-uploaddocument","Delete Upload Document - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname."  Patient 's ".date("d-m-Y h:i:s",strtotime($uploaddoc_DATA->date))."  ".$doc_type ."  Upload Document is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_lockedclinicalnotes()	{
    ...
    $lockedclinicalnotes_DATA = $this->db->query("select * from healthreport_lockedclinicalnotes where patient_id='$patient_id' and id='$patient_name'")->row();

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport - lockedclinicalnotes","Delete Locked Clinical Notes - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname."  Patient 's ".date("d-m-Y h:i:s",strtotime($lockedclinicalnotes_DATA->date))."  Locked Clinical Notes is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_signedconsentforms()	{
    ...
    $signedconsentform_DATA = $this->db->query("select * from healthreport_singedconsentforms where patient_id='$patient_id' and id='$patient_name'")->row();

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-singedconsentforms","Delete Singed Consent Forms - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname."  Patient 's ".date("d-m-Y h:i:s",strtotime($signedconsentform_DATA->date))."  Singed Consent Forms is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_doclabresult()	{
    ...
    $doclabresult_DATA = $this->db->query("select * from document_labresult where id='$patient_name'")->row();

    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentlabresult","Delete Document Lab Result - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname." 's  Patient ".date("d-m-Y h:i:s",strtotime($doclabresult_DATA->date))."  Document Lab Result is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_amendments()	{
    ...
    $docamendments_DATA = $this->db->query("select * from healthreport_amendments where id='$patient_name'")->row();

     $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentamendments","Delete Document Amendment - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname." 's  Patient ".date("d-m-Y h:i:s",strtotime($docamendments_DATA->date))."  Document Amendment is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function remove_healthrecord()	{
    ...
    $healthrecord_DATA = $this->db->query("select * from healthreport_healthrecord where healthrecord_id = '$healthrecord_id'")->row();
    ...
    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-healthrecord","Delete Health Record - Health Report",
            $get_patient_info->fname." ".$get_patient_info->lname." 's  Patient ".date("d-m-Y h:i:s",strtotime($healthrecord_DATA->date_time))."  Health Record is Delete by Admin at ".date('d-m-Y h:i:s') ,$this->session->userdata("hospital_id"),$patient_id,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
<a href="'.base_url().'/health_report/recordvaccination_print/'.$value->id.'" target="_blank" class="btn btn-xs icon-box"><i class="fa fa-print"></i></a>
	...
	public function create()
	{
		...
		$data['content'] = $this->load->view('healthreport_form',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	...
	public function save_vitalsing()
	{
		...
		if ($result) {
            ...
            $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vitalsing","Add Vital Sign - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Vital sign is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_vitalsing()
	{
        ...
        $query = $this->db->query("select patient_id,date from pa_vital_sign where patient_id='$patient_ID' and id='$id'");
        $no    = $query->num_rows();
        $result_pa_vital_sign = $query->row();
        $healthrecord_id = $result_pa_vital_sign->healthrecord_id;
        ...
        $vitasing_data = array("bloodtype"=>$bloodtype,"temperature"=>$temperature,"respiratory_rate"=>$respiratory_rate,"weight"=>$weight,"smoking_status"=>$smoking,"heart_rate"=>$heart_rate,"oxygen_saturation"=>$oxygen_saturation,"bmi"=>$bmi,"head_circumference"=>$head_circumfernce,"blood_pressure"=>$blood_presure,"height"=>$height,"pain"=>$pain,"glucose_by_glucometer"=>$glucose_glucometer,"date"=>date("Y-m-d H:i:s"));
		 ...
		 if ($no == 1) {
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vitalsing","Edit Vital Sign - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($result_pa_vital_sign->date))." Vital sign is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
              ...
              $vitasing_data = array("bloodtype"=>$bloodtype,"patient_id"=>$patient_ID,"healthrecord_id"=>$healthrecord_id,"temperature"=>$temperature,"respiratory_rate"=>$respiratory_rate,"weight"=>$weight,"smoking_status"=>$smoking,"heart_rate"=>$heart_rate,"oxygen_saturation"=>$oxygen_saturation,"bmi"=>$bmi,"head_circumference"=>$head_circumfernce,"blood_pressure"=>$blood_presure,"height"=>$height,"pain"=>$pain,"glucose_by_glucometer"=>$glucose_glucometer,"date"=>date("Y-m-d H:i:s"));
            $this->db->insert('pa_vital_sign',$vitasing_data);
             $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vitalsing","Add Vital Sign - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Vital sign is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_problemlist()
	{
		...
		if($problem!="" OR $icdversion!="" OR $icd10_code!="" OR $snomed_ct_code!="" OR $problem_status!="" OR $notes!=""){
            ...
            $diagnosis_datetime     = ($this->input->post("diagnosis_datetime")!=" ") ? date("Y-m-d h:i:s",strtotime($this->input->post("diagnosis_datetime"))) : "";
            $onset_datetime         = ($this->input->post("onset_datetime")!=" ") ?  date("Y-m-d h:i:s",strtotime($this->input->post("onset_datetime"))) : "";
            $channged_datetime      = ($this->input->post("channged_datetime")!=" ") ? date("Y-m-d h:i:s",strtotime($this->input->post("channged_datetime"))) : "";
            $session_healthrecord_id = $this->session->userdata("healthrecord_ids");
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-problemlist","Add Problem List - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." 's ".$this->input->post("problem")." problem of Health Report  - Problem List is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_problemlist()
	{
		...
		$notes					= $this->input->post("notes");
        $session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");

        $get_patient_info = $this->audit_patient_detail($patient_ID);
        $problem_detail = $this->db->query("select * from healthreport_problem where id='$healthreport_problem_id'")->row();
		...
		$query = $this->db->query("select patient_id from healthreport_problem where patient_id='$patient_ID' and id='$healthreport_problem_id'");
		$no    = $query->num_rows();
		if ($no == 1) {
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-problemlist","Edit Problem List - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." 's  ".$problem_detail->problem_id."   ".date('d-m-Y h:i:s',strtotime($problem_detail->date)). "  problem of Health Report  - Problem List is Edit by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
            ...
            $problemlist_data = array("patient_id"=>$patient_ID,"healthrecord_id"=>$session_healthrecord_id,"problem_id"=>$problem,"icd_version_id"=>$icdversion,"icd10_code"=>$icd10_code,"snomed_ct_code"=>$snomed_ct_code,"status"=>$problem_status,"diagnosis_datetime"=>$diagnosis_datetime,"onset_datetime"=>$onset_datetime,"changed_datetime"=>$channged_datetime,"notes"=>$notes,"date"=>date("Y-m-d H:i:s"));
            $result = $this->db->insert("healthreport_problem",$problemlist_data);
            if ($result) {
	...
	public function save_allergylist()
	{
		...
		if ($allergy_type!=""||$drug_allergy!=""||$reaction!=""||$severity!=""||$allergy_status!=""||$notes!="") {
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-allergylist","Add Allergy - Health Report",$get_patient_info->fname." ".$get_patient_info->lname."Patient 's " .$this->input->post("allergy_type").' : '.$this->input->post("drug_allergy")." Allergy  is Add by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	function edit_update_allergylist(){
        ...
        $query = $this->db->query("select patient_id,allergy_type,drug_allergy from healthreport_allergies where patient_id='$patient_ID' and id='$allergies_id'");
        $no    = $query->num_rows();
        $allergylist_result = $query->row();
        ...
        if ($no == 1) {
            ...
            if ($result) {
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-allergylist","Edit Allergy - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".$allergylist_result->allergy_type.' : '.$allergylist_result->drug_allergy." Allergy is Edit by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
                echo 1;exit;
            }else{
                echo 0;exit;
            }
        }else{
            $allergylist_data = array("patient_id"=>$patient_ID,"healthrecord_id"=>$session_healthrecord_id,"allergy_type"=>$allergy_type,"drug_allergy"=>$drug_allergy_e,"reaction"=>$reaction,"severity"=>$severity,"status"=>$allergy_status,"notes"=>$notes,"date"=>date("Y-m-d H:i:s"));
            $result = $this->db->insert("healthreport_allergies",$allergylist_data);
            if ($result) {
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-allergylist","Add Allergy - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".$this->input->post("allergy_type").' : '.$this->input->post("drug_allergy")." Allergy is Add by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_labresult()
	{
        ...
        $get_patient_info = $this->audit_patient_detail($patient_ID);
		 ...
		 if ($loinc_code!=""||$discription!=""||$result_value!=""||$units!=""||$normal_range!=""||$abnormal_flag!="") {
		     ...
		     $labresult_data = array("loinc_code"=>$loinc_code,"description"=>$discription,"result_value"=>$result_value,"units"=>$units,"normal_range"=>$normal_range,"abnormal_flag"=>$abnormal_flag,"patient_id"=>$patient_ID,"healthrecord_id"=>$session_healthrecord_id,"labresult_datetime"=>$l_date_time,"date"=>date("Y-m-d H:i:s"));
			$result = $this->healthreport_model->save_labresult($labresult_data);
			if ($result) {
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-labresult","Add Lab Result - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ". $this->input->post("loinc_code")." Lab Result is Add by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_labresult()
	{
        ...
        $query = $this->db->query("select patient_id,loinc_code,date from lab_result where patient_id='$patient_ID' and loinc_code_id='$healthreport_labresult_id'");
        $no    = $query->num_rows();
        $resultlabresult = $query->row();
        ...
        if ($no == 1) {
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-labresult","Edit Lab Result - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".date("d-m-Y h:i:s",strtotime($resultlabresult->date))." Lab Result is Edit by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
            ...
            $labresult_data = array("loinc_code"=>$loinc_code,"description"=>$discription,"result_value"=>$result_value,"units"=>$units,"normal_range"=>$normal_range,"abnormal_flag"=>$abnormal_flag,"patient_id"=>$patient_ID,"healthrecord_id"=>$session_healthrecord_id,"labresult_datetime"=>$l_date_time,"date"=>date("Y-m-d H:i:s"));
            $result = $this->db->insert("lab_result",$labresult_data);
            if ($result) {
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-labresult","Add Lab Result - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ". $this->input->post("loinc_code")." Lab Result is Add by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
                echo 11;exit;
                ...
                if ($_FILES["scanned_result"]["type"] == "application/pdf"  || $_FILES["scanned_result"]["type"] == "image/jpg" || $_FILES["scanned_result"]["type"] == "image/png"  || $_FILES["scanned_result"]["type"] == "image/jpeg") {
                    ...
                    if ($result) {
                        $this->do_upload($scanned_resultdocname,'./assets/scanned_result_document','pdf|jpg|png|jpeg',"scanned_result");
                         ...
                         $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-imagingorder","Add Imaging Order - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ". $this->input->post("cpt_code")." Imaging Order is Add by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_imagingorder()
	{
		...
		$discription		= $this->input->post("imaging_order_description");
		$order_status		= $this->input->post("imaging_orders_status");
		$healthreport_imaging_id	= ($this->input->post("healthreport_imaging_id")!="") ? $this->input->post("healthreport_imaging_id") : 0;
		$doctor_comments	= $this->input->post("doctor_comments");
        $session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");

        $get_patient_info = $this->audit_patient_detail($patient_ID);

        $sql = "select patient_id,date from imaging_order where patient_id='$patient_ID' and imaging_order_id='$healthreport_imaging_id'";
        ...
        if (!empty($query)) {
    		...
    		if(isset($_FILES["scanned_result"]["name"]) &&  !empty($_FILES["scanned_result"]["name"]))
    		{
    			if ($_FILES["scanned_result"]["type"] == "application/pdf"  || $_FILES["scanned_result"]["type"] == "image/jpg" || $_FILES["scanned_result"]["type"] == "image/png"  || $_FILES["scanned_result"]["type"] == "image/jpeg") {
    				...
    				if ($result) {
    					$this->do_upload($scanned_resultdocname,'./assets/scanned_result_document','pdf|jpg|png|jpeg',"scanned_result");
                        $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-imagingorder","Edit Imaging Order - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".$query->date." Imaging Order is Update by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
    					echo 1;exit;
    				}else{
    					echo "0";exit;
    				}
    			...
    			$imagingorder_data = array("cpt_code"=>$cpt_code,"description"=>$discription,"order_status"=>$order_status,"doctor_comments"=>$doctor_comments,"patient_id"=>$patient_ID,"date"=>date("Y-m-d H:i:s"));
    					  $this->db->where('imaging_order_id',$healthreport_imaging_id);
    			$result = $this->db->update('imaging_order',$imagingorder_data);
    	   		//echo $this->db->last_query();exit;
    	   		if ($result) {
	...
	{  // print_r($_POST);
		...
		if ($drug_name!=""|| $prn!=""|| $sign_note!=""|| $sign!=""|| $indication!=""|| $e_prescription_status!=""|| $appointment!="null" ||
            ...
            $prescribe_date_time    = ($this->input->post("prescribe_date_time") !="") ? date('Y-m-d h:i:s',strtotime($this->input->post("prescribe_date_time"))) : "";


            $started_taking_date_time    = ($this->input->post("started_taking_date_time")!=" ") ? date("Y-m-d h:i:s",strtotime($this->input->post("started_taking_date_time"))) : "";

            $stopped_taking_date_time   = ($this->input->post("stopped_taking_date_time")!=" ") ? date("Y-m-d h:i:s",strtotime($this->input->post("stopped_taking_date_time"))) : "";

            $e_prescription_data = array("drug_name"=>$drug_name,"sig"=>$sign,"datetime_stopped_taking"=>$stopped_taking_date_time,"number_refills"=>$number_refills,"pharmacy_note"=>$pharmacy_note,"notes"=>$notes,"prn"=>$prn,"indication"=>$indication,"datetime_prescribed"=>$prescribe_date_time,"dispense_quantity"=>$dispense_quantity,"daw"=>$daw,"order_status"=>$order_status,"sig_note"=>$sign_note,"status"=>$e_prescription_status,"datetime_started_taking"=>$started_taking_date_time,"dispense_package"=>$dispense_package,"appointment_id"=>$appointment,"patient_id"=>$patient_ID,"healthrecord_id"=>$session_healthrecord_id,"date"=>date("Y-m-d H:i:s"));
			$result = $this->healthreport_model->save_e_prescription($e_prescription_data);
			if ($result) {

                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-E-prescription/medication","Add E-prescription/medication - Health Report ",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".$this->input->post("drug_name")." E-prescription/Medication is Add by Admin at ".date("Y-m-d H:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
...
public function edit_e_prescription()
{
	...
	$order_status	= $this->input->post("order_status");
	$notes	= $this->input->post("notes");
	$e_prescription	= ($this->input->post("e_prescription")!="") ? $this->input->post("e_prescription") : 0;
    $session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");
    ...
    $query = $this->db->query("select * from e_prescription where patient_id='$patient_ID' and e_prescription_id='$e_prescription'");
    //echo $this->db->last_query();exit;
    $no    = $query->num_rows();
    $eprescription_DATA = $query->row();
    if ($no == 1) {
		$this->db->where('e_prescription_id',$e_prescription);
        $result =  $this->db->update('e_prescription',$e_prescription_data);
		if ($result) {

            $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"E-prescription/medication","Edit E-prescription/medication - Health Report",$get_patient_info->fname." ".$get_patient_info->lname." Patient 's ".date("d-m-Y h:i:s",strtotime($eprescription_DATA->date))." E-prescription / Medication is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
        ...
        if ($drug_name!=""|| $prn!=""|| $sign_note!=""||$e_prescription_status!=""|| $sign!=""|| $indication!=""|| $e_prescription_status!=""|| $appointment!="null" ||
        $dispense_quantity!="" || $dispense_package!="" || $number_refills!="" || $daw!="" || $pharmacy_note!="" || $order_status!="" || $notes!="") {
	...
	public function save_summary()
	{
		...
		if ($summary!="") {
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-summary","Add Summary - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Summary is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_summary()
	{
		$summary 	= $this->input->post("summary");
		$patient_ID 	= $this->input->post("patient_ID");
		$summary_id 	= ($this->input->post("summary_id")!="") ? $this->input->post("summary_id") : 0;
        $session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");

        $get_patient_info = $this->audit_patient_detail($patient_ID);
        $summary_DATA = $this->db->query("select * from  healthreport_summary where id = '$summary_id' ")->row();
        ...
        if (!empty($summary_DATA)) {
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-summary","Edit Summary - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($summary_DATA->date))." Summary is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_healthrecord(){
		...
		if ($result) {
            ...
            $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-healthrecord","Add Health Record - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Health Record is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_vaccines(){
		...
		if ($vaccines!="" || $schedule!="" || $vaccine!="" || $cvx_code!="" || $vis!="" || $administreted_on!="" || $vaccinestatus!="") {
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vaccines","Add Vaccines - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Vaccines is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
    ...
    public function edit_Vaccines()
    {
        ...
        $query = $this->db->query("select * from healthreport_vaccines where patient_id='$patient_ID' and id='$vaccines_e_ID'");
        $no  = $query->num_rows();
        $Vaccines_DATA = $query->row();
        ...
        if ($no == 1) {
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-vaccines","Edit Vaccines - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname. 's Patient  '.date("Y-m-d h:i:s",strtotime($Vaccines_DATA->date))." Health Report Vaccines is Edit by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
            ...
            $Vaccines_data = array("vaccines" => $vaccines,"schedule"=>$schedule,"vaccine"=>$vaccine,"cvxcode"=>$cvx_code,"vis"=>$vis,"administreted_on"=>$administreted_on,"administreted_by"=>$administreted_by,"vaccines_status"=>$vaccinestatus,"patient_id" => $patient_ID,"healthrecord_id"=>$session_healthrecord_id,"date"=>date("Y-m-d H:i:s"));
            $result = $this->db->insert('healthreport_vaccines', $Vaccines_data);
            // echo $this->db->last_query();exit;
            if ($result) {
	...
	public function save_record_vaccinations(){
		...
		if($create_record!="" ||$cvx_code!="" ||$name!=""|| $cpt_code!=""||$manfacturer!="" ||$lot_num!="" ||  $administered_amount!="" || $vaccinate_site!="" || $vaccinate_status!="" ||  $ordering_doctor!="" || $administered_at !="" || $inventory_lot!="" ||$record_type!="null" || $funding_eligibility!="null" || $observed_immunity!="null" || $record_vaccination_notes!="" || $administered_units!="" ){
			...
			if ($result) {
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-recordvaccinations","Add Record Vaccinations - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Record Vaccinations is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
				echo 1;exit;
			}else{
				echo 0;exit;
			}
	...
	public function edit_record_vaccinations(){
		...
		$record_vaccination_notes 	= $this->input->post("record_vaccination_notes");
		$administered_units         = $this->input->post("administered_unit");
		$record_vaccinations_id         = ($this->input->post("record_vaccinations_id") !="") ? $this->input->post("record_vaccinations_id") : 0;
        $session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");

        $get_patient_info = $this->audit_patient_detail($patient_ID);

        $result_record_vaccinations = $this->db->query("select * from healthreport_recordvaccinations where id = '$record_vaccinations_id' ")->row();

		$recordvaccinations = array("create_record_for"=>$create_record,"cvxcode"=>$cvx_code,"name"=>$name ,"cpt_code"=>$cpt_code,"manfacturer"=>$manfacturer,"expirationdate"=>$expirationdate,"lot_num"=>$lot_num,"administered_amount"=>$administered_amount,"vaccinate_route"=>$vaccinate_route,"vaccinate_route"=>$vaccinate_route,"vaccinate_site"=>$vaccinate_site,"vaccinate_status"=>$vaccinate_status,"administred_on"=>$administred_on,"ordering_doctor"=>$ordering_doctor,"administered_by"=>$administered_by,"administered_at"=>$administered_at,"inventory_lot"=>$inventory_lot,"record_type"=>$record_type,"funding_eligibility"=>$funding_eligibility,"observed_immunity"=>$observed_immunity,"record_vaccination_notes"=>$record_vaccination_notes,"administered_units"=>$administered_units,"patient_id"=>$patient_ID,"date"=>date("Y-m-d H:i:s"));
            //print_r($recordvaccinations);exit;
        $query = $this->db->query("select patient_id from healthreport_recordvaccinations where patient_id='$patient_ID' and id='$record_vaccinations_id'")->row();
        //print_r($query);exit;
        if (!empty($query)) {
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-recordvaccinations","Edit Record Vaccinations - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($result_record_vaccinations->date))." Record Vaccinations is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
           ...
           if($create_record!="" ||$cvx_code!="" ||$name!=""|| $cpt_code!=""||$manfacturer!="" ||$lot_num!="" ||  $administered_amount!="" || $vaccinate_site!="" || $vaccinate_status!="" ||  $ordering_doctor!="" || $administered_at !="" || $inventory_lot!="" ||$record_type!="null" || $funding_eligibility!="null" || $observed_immunity!="null" || $record_vaccination_notes!="" || $administered_units!="" ){
                ...
                $result = $this->db->insert("healthreport_recordvaccinations",$recordvaccinations);
                if ($result) {
                    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-recordvaccinations","Add Record Vaccinations - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Record Vaccinations is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_clinical_notes(){
		...
		if ( $locked_by!="" || $locked_on!="" || $lockedaction!="" || $lockedstatus!="" || $lockedreason!="") {
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport- lockedclinicalnotes","Add Clinical Notes - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Locked Clinical Notes is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_clinical_notes(){
        ...
        $healthrecord_id = $this->session->userdata("edit_healthrecord_ids");
		...
		//print_r($clinical_notes);exit;
        $sql = "select * from healthreport_lockedclinicalnotes where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
        $ex = $this->db->query($sql);
		$row = $ex->row();
        $num    = $ex->num_rows();
		if(empty($row)){
			//echo "Hi";exit;
			$clinical_notes = array("patient_id"=>$patient_ID,"healthrecord_id"=>$healthrecord_id,"date_of_service"=>$date_of_service,"locked_by"=>$locked_by,"locked_on"=>$locked_on,"action"=>$lockedaction ,"status"=>$lockedstatus,"reason"=>$locked_reason,"date"=>date("Y-m-d H:i:s"));
            $result =  $this->db->insert('healthreport_lockedclinicalnotes',$clinical_notes);
			//echo $this->db->last_query();exit;
            if ($result) {
			...
			//echo "Hiiiiiiiiiiii";exit;
				        $this->db->where('id',$locked_id);
            $result =  $this->db->update('healthreport_lockedclinicalnotes',$clinical_notes);
            echo $this->db->last_query();
            if ($result) {

                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport - lockedclinicalnotes","Edit Clinical Notes - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($row->date))." Locked Clinical Notes is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_singed_consent(){
		...
		if($singnature_action !=""){
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-singedconsentforms","Add Singed Consent Forms- Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Singed Consent Forms is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_singed_consent(){
		...
		$singed_consent = array("patient_id"=>$patient_ID,"appointment_datetime"=>$appointment_datetime,"singnature_date"=>$singnature_date,"singnature_action"=>$singnature_action ,"date"=>date("Y-m-d H:i:s"));
		$sql = "select * from healthreport_singedconsentforms where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
        $ex = $this->db->query($sql);
        $row = $ex->row();
        $num    = $ex->num_rows();
        if(empty($row)){
            ...
            $singed_consent = array("patient_id"=>$patient_ID,"healthrecord_id"=>$healthrecord_id,"appointment_datetime"=>$appointment_datetime,"singnature_date"=>$singnature_date,"singnature_action"=>$singnature_action ,"date"=>date("Y-m-d H:i:s"));
            //print_r($singed_consent);exit;
            $result = $this->healthreport_model->save_singed_consent($singed_consent);
            if ($result) {
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-singedconsentforms","Add Singed Consent Forms- Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Singed Consent Forms is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
                echo 11;exit;
            ...
            $result = $this->db->update('healthreport_singedconsentforms',$singed_consent);
            if ($result) {

                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-singedconsentforms","Edit Singed Consent Form - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($row->date))." Singed Consent Form  is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function save_lab_result(){
		...
		if($lab !="" || $abnormal !="" ||  $action !="" || $result_count !="" || $test!=""){
			...
			if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentlabresult","Add Document Lab Result - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Document Lab Result is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_lab_result(){
        ...
        $healthrecord_id = $this->session->userdata("edit_healthrecord_ids");
		$lab_result = array("patient_id"=>$patient_ID,"lab"=>$lab,"date"=>$date,"action"=>$action,"abnormal"=>$abnormal ,"result_count"=>$result_count,"test"=>$test,"date"=>date("Y-m-d H:i:s"));

        $sql = "select * from document_labresult where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
        $ex = $this->db->query($sql);
        $row = $ex->row();
        $num    = $ex->num_rows();
        ...
        if($num == 1){
            ...
            if ($result) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentlabresult","Edit Document Lab Result - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($row->date))." Document Lab Result is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
            ...
            $lab_result = array("patient_id"=>$patient_ID,"healthrecord_id"=>$healthrecord_id,"lab"=>$lab,"date"=>$date,"action"=>$action,"abnormal"=>$abnormal ,"result_count"=>$result_count,"test"=>$test,"date"=>date("Y-m-d H:i:s"));
            //print_r($lab_result);exit;
            $result = $this->healthreport_model->save_lab_result($lab_result);
            if ($result) {
	...
	public function save_amendments() {
		...
		if($amendment_source !="" || $amendment_status !="" ){
			...
			if ($sql) {
                ...
                $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentamendments","Add Document Amendments - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Document Amendments is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_amendments() {
        ...
        $sql = "select patient_id from healthreport_amendments where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
        $ex = $this->db->query($sql);
        $row = $ex->row();
        $num    = $ex->num_rows();
        if($num == 1){
            $amdments_date_time         = ($this->input->post('amdments_date_time_e')!= "-0001-11-30 12:00" ) ? date("Y-m-d h:i:s",strtotime($this->input->post('amdments_date_time_e'))) : "";
             $amdmentsproccess_date_time = ($this->input->post('amdmentsproccess_date_time_e')!="") ? date("Y-m-d h:i:s",strtotime($this->input->post('amdmentsproccess_date_time_e'))) : "";
			$data = array("patient_id"=>$patient_ID,"amendment_source"=>$amendment_source,"amendment_status"=>$amendment_status,"amdments_date_time"=>$amdments_date_time,"amdmentsproccess_date_time"=>$amdmentsproccess_date_time,"date"=>date("Y-m-d H:i:s"));

			$this->db->where('id',$amdments_id);
		    $sql =	$this->db->update('healthreport_amendments',$data);
			if ($sql) {
                 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentamendments","Edit Document Amendments - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Document Amendments is Edit by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
            ...
            if($amendment_source !="" || $amendment_status !="" ){
                ...
                $data = array("patient_id"=>$patient_ID,"healthrecord_id"=>$healthrecord_id,"amendment_source"=>$amendment_source,"amendment_status"=>$amendment_status,"amdments_date_time"=>$amdments_date_time,"amdmentsproccess_date_time"=>$amdmentsproccess_date_time,"date"=>date("Y-m-d H:i:s"));
                $sql = $this->db->insert("healthreport_amendments",$data);
                if ($sql) {
                     $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-documentamendments","Add Document Amendments - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname." Health Report Document Amendments is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
		...
		if(isset($_FILES["document_document"]["name"]))
        {
			if ($_FILES["document_document"]["type"] == "application/pdf"  || $_FILES["document_document"]["type"] == "image/jpg" || $_FILES["document_document"]["type"] == "image/png"  || $_FILES["document_document"]["type"] == "image/jpeg") {
				...
				if ($sql) {
                    ...
                    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-uploaddocument","Add Upload Document - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".$doc_type." - Health Report  Upload Document  is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
	...
	public function edit_document_form() {
        ...
        $query = $this->db->query("select * from healthreport_document where patient_id='$patient_ID' and id='$healthreport_document_id' and document_type='$document_type'");
        $no    = $query->num_rows();
        $resultuplod_doc = $query->row();
        if($no == 1){
    		if(isset($_FILES["document_document_e"]["name"]) && $_FILES["document_document_e"]["name"]!="")
            {
    			if ($_FILES["document_document_e"]["type"] == "application/pdf"  || $_FILES["document_document_e"]["type"] == "image/jpg" || $_FILES["document_document_e"]["type"] == "image/png"  || $_FILES["document_document_e"]["type"] == "image/jpeg") {
    				...
    				if ($sql) {
                        ...
                        $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-uploaddocument","Edit Upload Document - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($resultuplod_doc->date))."  ".$doc_type ." - Upload Document is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
                ...
                $document_docname = $this->input->post("old_uploaded_document");
                $data = array("patient_id"=>$patient_ID,"document_document"=>$document_docname,"document_type"=>$document_type,"date"=>date("Y-m-d H:i:s"));
                ...
                if ($sql) {
                    ...
                    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-uploaddocument","Edit Upload Document - Health Report",
                $get_patient_info->fname." ".$get_patient_info->lname."  ".date('d-m-Y h:i:s',strtotime($resultuplod_doc->date))." Upload Document is Edit by Admin at ".date('d-m-Y h:i:s'),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
		...
		if(isset($_FILES["review_document"]["name"]))
        {
			if ($_FILES["review_document"]["type"] == "application/pdf"  || $_FILES["review_document"]["type"] == "image/jpg" || $_FILES["review_document"]["type"] == "image/png"  || $_FILES["review_document"]["type"] == "image/jpeg") {
				...
				if ($sql) {
                    ...
                    $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-reviewsing","Add Review Sign - Health Report",
                    $get_patient_info->fname." ".$get_patient_info->lname." Health Report Review Sign is Add by Admin at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
				}
			}else{
				echo "0";exit;
			}
	...
	public function update_review_sing(){
	//  print_r($_SESSION);
	//  exit;
		$patient_ID             = $this->input->post('patient_ID');
		$printname              = $this->input->post('printname_e');
		$singnature     =  $_REQUEST['singnatureImage_e'];
		$data = str_replace('data:image/png;base64,', '', $singnature);
		$data = str_replace(' ', '+', $data);
		$data = base64_decode($data);
		$file =     'assets/reviewsing_document/'.rand().'_reviewsing'.'.png';
		...
		$session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");
		$get_patient_info = $this->audit_patient_detail($patient_ID);
		if(isset($_FILES["review_document_e"]["name"]))
		{
			if ($_FILES["review_document_e"]["type"] == "application/pdf"  || $_FILES["review_document_e"]["type"] == "image/jpg" || $_FILES["review_document_e"]["type"] == "image/png"  || $_FILES["review_document_e"]["type"] == "image/jpeg") {
				$reivew_doc = $_FILES["review_document_e"]["name"];
				$reivew_docname = rand(999,9999)."_reviewsingdocument".".".pathinfo($reivew_doc, PATHINFO_EXTENSION);
	$data=array("patient_id"=>$patient_ID,"healthrecord_id"=>$session_healthrecord_id,"reviewsing_document"=>$reivew_docname,"printname"=>$printname,"signture_image"=>$file,"date"=>date("Y-m-d H:i:s"));
			...
			//  exit;
				if ($sql) {
					$this->do_upload($reivew_docname,'./assets/reviewsing_document','pdf|jpg|png|jpeg',"review_document_e");
					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"healthreport-reviewsing","Add Review Sign - Health Report",
					$get_patient_info->fname." ".$get_patient_info->lname." Health Report Review Sign is Add by Medical Provider at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_ID,$get_patient_info->fname." ".$get_patient_info->lname,10);
					}
					}else{
						echo "0";exit;
						}
    ...
    public function all_report_pdf($healthrecord_id)
    {
        $this->load->library('dompdf_gen');
        $customPaper = array(0,0,1024,1000);
        $this->dompdf->set_paper($customPaper);
        $vitalsing = $this->db->select("*")->from("pa_vital_sign")->where("healthrecord_id",$healthrecord_id)->get()->row();
        //echo $this->db->last_query();exit;
        $patient_name = $this->db->select("*")->from("patient")->where("id",$vitalsing->patient_id)->get()->row();
        $pdfname = "Patient" . date('YmdHis') . '.pdf';
        $html = '<style>
                                table {
                                    display: table; border-collapse: collapse;
                        ...
                        <h3 style="text-align:center;">Health Report For Vital Sign </h3>
                        <p>Patient Name :'.$patient_name->fname.' '.$patient_name->lname.'</p>
                        <table border="1" width="100%" class="pricedetail" style="margin-top: 1px;margin-bottom:20px;">
                                 <tr>
                                        <th>Temperature</th>
                                        <th>Respiratory Rate</th>
                                        <th>Weight</th>
                                ...
                                $html .='<tr>
                                        <td width="10%" style="padding: 5px;">'.$vitalsing->temperature.'</td><td style="padding: 5px;">'.$vitalsing->respiratory_rate.'</td>

                                        <td style="padding: 5px;">'.$vitalsing->weight.'</td><td style="padding: 5px;">'.$vitalsing->smoking_status.'</td>
                                        <td align="left" style="width:16%;padding: 5px;">'.$vitalsing->heart_rate.'
                                        </td>
                                        <td width="12%" style="padding: 5px;">'.$vitalsing->oxygen_saturation.'</td>';

                                        $html .='<td style="padding: 5px;">'.$vitalsing->bmi.'</td>';

                                            $html .='<td style="padding: 5px;">'.$vitalsing->head_circumference.'</td>';


                                            $html .='<td style="padding: 5px;">'.$vitalsing->blood_pressure.'</td>';


                                            $html .='<td style="padding: 5px;">'.$vitalsing->height.'</td>';

                                        $html .='<td style="padding: 5px;">'.$vitalsing->pain.'</td>';
                                        $html .='<td style="padding: 5px;">'.$vitalsing->glucose_by_glucometer.'</td>';
                                        $html .='<td style="padding: 5px;">'.$vitalsing->bloodtype.'</td>';
                                $html .='</tr>';


                        $html .='</table>';
                        $problem = $this->db->select("*")->from("healthreport_problem")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($problem)){
                            $html .= '</br></br></br>
                                    <h3 style="text-align:center;">Health Report For Problem </h3>
                                    <table border="1" width="100%" class="pricedetail" style="margin-top: 5px;margin-bottom:20px;">
                                        <tr>
                                            <th>Problem</th>
                                            <th>Icd version</th>
                                            <th>Icd10 Code</th>
                                ...
                                $html .='<tr>
                                        <td width="10%" style="padding: 5px;">'.$problem->problem_id.'</td><td style="padding: 5px;">'.$problem->icd_version_id.'</td>

                                        <td style="padding: 5px;">'.$problem->icd10_code.'</td><td style="padding: 5px;">'.$problem->snomed_ct_code.'</td>
                                        <td align="left" style="width:16%;padding: 5px;">'.$problem->status.'
                                        </td>
                                        <td width="12%" style="padding: 5px;">'.$problem->diagnosis_datetime.'</td>';

                                        $html .='<td style="padding: 5px;">'.$problem->onset_datetime.'</td>';

                                        $html .='<td style="padding: 5px;">'.$problem->changed_datetime.'</td>';
                                        $html .='<td style="padding: 5px;">'.$problem->notes.'</td>';
                                $html .='</tr>';
                                $html .='</table>';
                        }
                        $allergy = $this->db->select("*")->from("healthreport_allergies")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($allergy)){
                                $html .= '</br></br></br>
                                            <h3 style="text-align:center;margin-top:10px;">Health Report For Allergy </h3>
                                            <table border="1" width="100%" class="pricedetail" style="margin-top: 5px;margin-bottom:20px;">
                                             <tr>
                                                <th>Allergy Type</th>
                                                <th>Drug Allergy</th>
                                            ...
                                            $html .='<tr>
                                                    <td width="10%" style="padding: 5px;">'.$allergy->allergy_type.'</td><td style="padding: 5px;">'.$allergy->drug_allergy.'</td>

                                                    <td style="padding: 5px;">'.$allergy->reaction.'</td><td style="padding: 5px;">'.$allergy->severity.'</td>
                                                    <td align="left" style="width:16%;padding: 5px;">'.$allergy->status.'
                                                    </td>
                                                    <td width="12%" style="padding: 5px;">'.$allergy->notes.'</td>';
                                            $html .='</tr>';

                                $html .='</table>';
                        }
                        $lab_result = $this->db->select("*")->from("lab_result")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($lab_result)){
                                $html .='</br></br></br>
                                <h3 style="text-align:center;margin-top:10px;">Health Report For Lab Result</h3>
                                <table border="1" width="100%" class="pricedetail" style="margin-top: 1px;margin-bottom:20px;">
                                         <tr>
                                            <th>Loinc Code</th>
                                            <th>Description</th>
                                ...
                                $html .='<tr>
                                            <td width="10%" style="padding: 5px;">'.$lab_result->loinc_code.'</td><td style="padding: 5px;">'.$lab_result->description.'</td>
                                            <td style="padding: 5px;">'.$lab_result->result_value.'</td><td style="padding: 5px;">'.$lab_result->units.'</td>
                                            <td align="left" style="width:16%;padding: 5px;">'.$lab_result->normal_range.'
                                            </td>
                                            <td width="12%" style="padding: 5px;">'.$lab_result->abnormal_flag.'</td>';
                                    $html .='</tr>';
                                $html .='</table>';
                        }
                        $imaging_order = $this->db->select("*")->from("imaging_order")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($imaging_order)){
                            $html .='</br></br></br>
                            <h3 style="text-align:center;margin-top:10px;">Health Report For Imaging Order</h3>
                            <table border="1" width="100%" class="pricedetail" style="margin-top: 1px;margin-bottom:20px;">
                                <tr>
                                    <th>Cpt Code</th>
                                    <th>Description</th>
                            ...
                            $html .='<tr>
                                        <td width="10%" style="padding: 5px;">'.$imaging_order->cpt_code.'</td><td style="padding: 5px;">'.$imaging_order->description.'</td>
                                        <td style="padding: 5px;">'.$imaging_order->order_status.'</td><td style="padding: 5px;">'.$imaging_order->doctor_comments.'</td>';
                                    $html .='</tr>';
                            $html .='</table>';
                        }

                    $e_prescription = $this->db->select("*")->from("e_prescription")->where("healthrecord_id",$healthrecord_id)->get()->row();
                if(!empty($e_prescription)){
                            $html .= '</br></br></br>
                                      <h3 style="text-align:center;margin-top:10px;">Health Report For E-prescription/Medication</h3>
                                    ...
                                    $html .='<tr>
                                        <td width="10%" style="padding: 5px;">'.$e_prescription->drug_name.'</td><td style="padding: 5px;">'.$patient->sig.'</td>

                                        <td style="padding: 5px;">'.$e_prescription->datetime_stopped_taking.'</td>

                                        <td style="padding: 5px;">'.$e_prescription->number_refills.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->pharmacy_note.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->notes.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->prn.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->indication.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->datetime_prescribed.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->dispense_quantity.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->daw.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->order_status.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->sig_note.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->status.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->datetime_started_taking.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->dispense_package.'</td>';
                                        $html .='<td style="padding: 5px;">'.$e_prescription->date.'</td>';
                                    $html .='</tr>';
                            $html .='</table>';
                    }
                        $recordvaccinations = $this->db->select("*")->from("healthreport_recordvaccinations")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($recordvaccinations)){

                            $create_record_for = $this->db->select("*")->from("patient")->where("id",$recordvaccinations->create_record_for)->get()->row();
                            $html .= '</br></br></br>
                            <h3 style="text-align:center;margin-top:10px;">Health Report For Record Vaccinations</h3>
                            <table border="1" width="100%" class="pricedetail" style="margin-top: 5px;margin-left:-30px;margin-bottom:20px;">
                                 <tr>
                                    <th>Create <br> Record For</th>
                                    <th>Cvx Code</th>
                                    <th>Name</th>
                            ...
                            $html .='<tr>
                                    ...
                                    <td style="padding: 5px;">'.$recordvaccinations->cvxcode.'</td>

                                    <td style="padding: 5px;">'.$recordvaccinations->name.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->cpt_code.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->manufacture.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->expirationdate.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->lot_num.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->administered_amount.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->vaccinate_route.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->vaccinate_site.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->vaccinate_status.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->administred_on.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->ordering_doctor.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->administered_by.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->administered_at.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->inventory_lot.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->record_type.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->funding_eligibility.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->observed_immunity.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->record_vaccination_notes.'</td>';
                                    $html .='<td style="padding: 5px;">'.$recordvaccinations->administered_units.'</td>';
                            $html .='</tr>';
                            $html .='</table>';
                        }
                        $summary = $this->db->select("*")->from("healthreport_summary")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($summary)){
                            $html .='</br></br></br>   <h3 style="text-align:center;margin-top:10px;">Health Report For Summary</h3>
                                        <table border="1" width="100%" class="pricedetail" style="margin-top: 10px;margin-bottom:20px;">
                                            <tr>
                                                <th>Summary</th>
                                            </tr>';
                                        $html .='<tr>
                                            <td width="10%" style="padding: 5px;">'.$summary->summary.'</td>';
                                        $html .='</tr>';
                            $html .='</table>';
                        }
                        $lockedclinicalnotes = $this->db->select("*")->from("healthreport_lockedclinicalnotes")->where("healthrecord_id",$healthrecord_id)->get()->row();
                        if(!empty($lockedclinicalnotes)){

                                $html .='</br></br></br>   <h3 style="text-align:center;margin-top:10px;">Health Report For Locked Clinical Notes</h3>
                                        <table border="1" width="100%" class="pricedetail" style="margin-top: 10px;margin-bottom:20px;">
                                            <tr>
                                        ...
                                        $status = ($lockedclinicalnotes->status=="1")?"Active":"Inactive";
                                        $html .='<tr>
                                                <td width="10%" style="padding: 5px;">'.$lockedclinicalnotes->date_of_service.'</td>

                                                <td style="padding: 5px;">'.$lockedclinicalnotes->locked_by.'</td>

                                                <td style="padding: 5px;">'.$lockedclinicalnotes->locked_on.'</td>';
                                                $html .='<td style="padding: 5px;">'.$lockedclinicalnotes->action.'</td>';
                                                $html .='<td style="padding: 5px;">'.$status.'</td>';
                                                $html .='<td style="padding: 5px;">'.$lockedclinicalnotes->reason.'</td>';
                                                $html .='<td style="padding: 5px;">'.$lockedclinicalnotes->date.'</td>';

                                        $html .='</tr>';
                                $html .='</table>';
                            }

                            $singedconsentforms = $this->db->select("*")->from("healthreport_singedconsentforms")->where("healthrecord_id",$healthrecord_id)->get()->row();
                            if (!empty($singedconsentforms)) {
                                $html .='</br></br></br>   <h3 style="text-align:center;margin-top:10px;">Health Report For Singed Consent
                                        Forms</h3>
                                        <table border="1" width="100%" class="pricedetail" style="margin-top: 10px;margin-bottom:20px;">
                                            <tr>
                                                <th>Consent Form</th>
                                        ...
                                        $html .='<tr>
                                                <td width="10%" style="padding: 5px;">'.$singedconsentforms->consent_form.'</td>
                                                <td style="padding: 5px;">'.date('Y-m-d',strtotime($singedconsentforms->appointment_datetime)).'</td>
                                                <td style="padding: 5px;">'.date('Y-m-d',strtotime($singedconsentforms->singnature_date)).'</td>';
                                                $html .='<td style="padding: 5px;">'.$singedconsentforms->date.'</td>';

                                        $html .='</tr>';
                                $html .='</table>';
                            }
                            $document_labresult = $this->db->select("*")->from("document_labresult")->where("healthrecord_id",$healthrecord_id)->get()->row();
                            if(!empty($document_labresult)){
                                $html.='</br></br></br>   <h3 style="text-align:center;margin-top:10px;">Health Report For Lab Result</h3>
                                ...
                                $html .='<tr>
                                        <td width="10%" style="padding: 5px;">'.$document_labresult->lab.'</td>
                                        <td style="padding: 5px;">'.$document_labresult->date.'</td>
                                        <td style="padding: 5px;">'.$document_labresult->action.'</td>';
                                        $html .='<td style="padding: 5px;">'.$document_labresult->abnormal.'</td>';
                                        $html .='<td style="padding: 5px;">'.$document_labresult->result_count.'</td>';
                                        $html .='<td style="padding: 5px;">'.$document_labresult->test.'</td>';
                                        $html .='<td style="padding: 5px;">'.$document_labresult->date.'</td>';
                                $html .='</tr>';
                                $html .='</table>';

                            }

                            $amdments = $this->db->select("*")->from("healthreport_amendments")->where("healthrecord_id",$healthrecord_id)->get()->row();
                            if(!empty($amdments)){
                                $html.='</br></br></br>   <h3 style="text-align:center;">Health Report For Amendment</h3>
                                ...
                                $html .='<tr>
                                        <td width="10%" style="padding: 5px;">'.$amdments->amendment_source.'</td>
                                        <td style="padding: 5px;">'.$amdments->amendment_status.'</td>
                                        <td style="padding: 5px;">'.$amdments->amdments_date_time.'</td>';
                                $html .='<td style="padding: 5px;">'.$amdments->amdmentsproccess_date_time.'</td>';
                                $html .='<td style="padding: 5px;">'.$amdments->date.'</td>';
                                $html .='</tr>';
                                $html .='</table>';
                            }
