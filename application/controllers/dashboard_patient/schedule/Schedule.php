...
//exit;
		...
		if ($this->schedule_model->create($postData)) {
				...
				$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"schedule","Schedule Create",$patientdetail->fname."  ".$patientdetail->lname." Patient 's Scheduled Appointment at "." ".date('h:i:s',strtotime($from_time))." To ".date('h:i:s',strtotime($to_time)),$this->session->userdata("hospital_id"),$this->input->post('doctor_id',true),$doctordetail->firstname."  ".$doctordetail->lastname,2);
