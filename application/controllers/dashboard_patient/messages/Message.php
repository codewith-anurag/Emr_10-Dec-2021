	        ...
	        if($to_user_val == 'medical'){
		        // if($isadmin==1){
		           // $medical_drop = $this->db->select("*")->from("user")->where("created_by",$created_by_id)->where("user_role",'2')->where("find_in_set('message', features)")->where_not_in("email",$ignore)->get()->result();
							  $medical_drop = $this->db->select("*")->from("user")->where("hospital_id",$id)->where("user_role",'2')->where("find_in_set('message', features)")->where_not_in("email",$ignore)->get()->result();
//echo $this->db->last_query();
//print_r($_SESSION);
				...
				//if ($this->form_validation->run() === true) {
						if ($this->message_model->create($postData)) {
							...
							if($this->input->post('message_to_user') == "admin"){
								...
								$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"message","message send to admin",$patientdetail->fname."  ".$patientdetail->lname." Patient Message send to ".$adminData->firstname."  ".$adminData->lastname." admin.",$this->session->userdata("hospital_id"),$adminData->user_id,$adminData->firstname."  ".$adminData->lastname,1);
								...
								$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"message","message send to medical provider",$patientdetail->fname."  ".$patientdetail->lname." Patient Message send to ".$medicalproviderData->firstname."  ".$medicalproviderData->lastname." Medocal Provider.",$this->session->userdata("hospital_id"),$medicalproviderData->user_id,$medicalproviderData->firstname."  ".$medicalproviderData->lastname,2);
							}
							#set success message
							$this->session->set_flashdata('message', "Message send successfully");
						} else {
			...
			//	if ($this->form_validation->run() === true) {
						if ($this->message_model->create($postData)) {
							...
							if($this->input->post('message_to_user') == "admin"){
								...
								$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"message","message forward to medical provider",$patientdetail->fname."  ".$patientdetail->lname." Patient Message forward to ".$medicalproviderData->firstname."  ".$medicalproviderData->lastname." Medocal Provider.",$this->session->userdata("hospital_id"),$medicalproviderData->user_id,$medicalproviderData->firstname."  ".$medicalproviderData->lastname,2);
							}
	...
	public function areadmessage()
 {
    ...
    public function delete($id)
    {
        ...
        $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"message","message delete",$patientdetail->fname."  ".$patientdetail->lname." Patient Delete Message at ".date("Y-m-d h:i:s"),$this->session->userdata("hospital_id"),$patient_id,$patientdetail->fname."  ".$patientdetail->lname,10);
