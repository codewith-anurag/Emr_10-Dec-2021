...
class Item_master extends CI_Controller {
	...
	public function __construct()
	{
		...
		if($this->uri->segment(2) == "auto_login"){
		...
		}else if ($this->session->userdata('isLogIn') == false || $this->session->userdata('user_role') != 10){
			redirect('login');
		}
	}
 	public function auto_login(){ 
 		...
 		$userdata      = $this->db->get_where("patient",array("id"=>$patient_id))->row();
 			//print_r($userdata);
					...
					//check_user_status
						...
						if ($check_user->num_rows() === 1) {
								...
								$this->session->set_userdata([
										...
										'fullname'  => $check_user_data->firstname.' '.$check_user_data->lastname,
								...
								//print_r($check_user_data);exit;
								if($check_user_data->user_role==10){
									
									$this->session->set_userdata(['user_role' => 10]);
									...
									redirect("item_master/success_payment/".$patient_id."/".$orderID."/".$login_id.'/'.$payment_id);
								}
	...
	public function billing(){
		...
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"billing","Bill Not Paid",$patientdetail->fname."  ".$patientdetail->lname." Patient 's Bill Payment fail.",$this->session->userdata("hospital_id"),$patient_id,$patientdetail->fname."  ".$patientdetail->lname,10);
	...
	public function invoice(){
		...
		$data['content']  = $this->load->view('dashboard_patient/invoice/invoice' ,$data,true);
		...
		if($this->db->affected_rows()){
			if($payment_type == 1){
		    	...
		    	$data = '{
		            ...
		            "redirect_urls": {
		                "cancel_url":"https://emr.gthealthsystem.com/item_master/billing",
		                "return_url":"https://emr.gthealthsystem.com/dashboard_patient/invoice/item_master/auto_login/'.$patient_id.'/'.$orderID.'/'.$login_id.'"
		            }
		       }'; 
			   ...
			   /* echo  "<pre>";
				...
				$this->db->where("patient_id",$patient_id);
				$this->db->update("subscribe",array("status"=>1,"payment_id"=>$payment_Id,"updated_date"=>date("Y-m-d h:i:s")));

		$data['content']  = $this->load->view('success_payment' ,$data,true);
		$this->load->view('layout/main_wrapper',$data);	

		$patientDetail = $this->db->get_where("patient",array("id"=>$patient_id))->row();
		$subscribe_detail = $this->db->get_where("subscribe",array("id"=>$subscribe_id,"status"=>1,"payment_type"=>$payment_type))->row();	
		//print_r($subscribe_detail);
		$order = $this->db->get_where("subscribe",array("order_id"=>$subscribe_detail->order_id))->row();
		//print_r($order);		
		$orderID = $subscribe_detail->order_id;
		...
		$logo = ($hospital_name->picture != "") ? base_url().$hospital_name->picture : base_url()."assets/images/logo.png";
    		...
    		//print_r($result);exit;
			   ...
			   /* $to = $patientDetail->email;
				...
				//redirect(base_url('dashboard_patient/invoice/item_master/invoice'));
...
<div style="border:1px solid #ccc !important; padding: 25px !important; margin: 25px !important;">
 ...
 //echo $html;exit;
	...
	public function success_payment($patient_id="",$orderID="",$login_id="",$payment_Id="")
	{
		...
		$subscribe_detail = $this->db->get_where("subscribe",array("id"=>$order->id,"status"=>1))->row();				
