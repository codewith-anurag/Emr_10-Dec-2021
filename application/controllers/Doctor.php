...
class Doctor extends CI_Controller {
	...
	public function changestatus()
	{
		...
		$medicalprodireDetail = $this->db->query("select * from user where user_id='$id'")->row();
		 ...
		 $audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"medicalprovider","Change Status of Medical Provider",$medicalprodireDetail->firstname."  ".$medicalprodireDetail->lastname."  is ".$status_value."  by  Admin.",$this->session->userdata("hospital_id"),$medicalprodireDetail->user_id,$medicalprodireDetail->firstname."  ".$medicalprodireDetail->lastname,2);
    ...
    //$datas['password'] = md5($newpass);
		...
		$this->form_validation->set_rules('department_id', 'Department','required|max_length[10]');
		$this->form_validation->set_rules('role_id', 'Role','required|max_length[10]');
    ...
    #other department add
		...
		if ($this->input->post('user_id',true) == null) {
			$data['doctor'] = (object)$postData = [
				...
				'created_by'   => $session_id,//$this->session->userdata('user_id'),
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['user_id'])) {
				if ($this->doctor_model->create($postData)) {
					$medicalprovider_id  = $this->db->insert_id();
					#set success message
...
<tr>
 <td width="20%"></td>
	...
	<td width="20%"></td>
					...
					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"medicalprovider","Add Medical Provider",$this->input->post('firstname',true).' '.$this->input->post('lastname',true)." Medical Provider is Add by  Admin.",$this->session->userdata("hospital_id"),$medicalprovider_id,$this->input->post('firstname',true).' '.$this->input->post('lastname',true),2);
				...
				if ($this->doctor_model->update($postData)) {
					$medicalprovider_id = $postData['user_id'];
					$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"medicalprovider","Update Medical Provider",$this->input->post('firstname',true).' '.$this->input->post('lastname',true)." Medical Provider is Update by  Admin.",$this->session->userdata("hospital_id"),$medicalprovider_id,$this->input->post('firstname',true).' '.$this->input->post('lastname',true),2);
...
// 	 }
	...
	public function delete($user_id = null)
	{
		...
		$medicalprodireDetail = $this->db->query("select * from user where user_id='$user_id'")->row();
		$audit_success = insert_auditdump($this->session->userdata("user_id"),$this->session->userdata("user_role"),"medicalprovider","Delete Medical Provider",$medicalprodireDetail->firstname."  ".$medicalprodireDetail->lastname."  is Delete by  Admin.",$this->session->userdata("hospital_id"),$user_id,$medicalprodireDetail->firstname."  ".$medicalprodireDetail->lastname,2);
