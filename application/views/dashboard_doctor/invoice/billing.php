...
<div class="col-lg-12 white-box" style="width: 100%;margin-right: 2%;min-height: 850px;">
  <section class="box">
    ...
    <div class="col-lg-12" style="min-height: 850px;">
      <table class="table billingDTable" id="padd">
        <thead>
          <tr>
        ...
        ?>
          <tr class="hovertr">            
            <td class="text-left"><a href="<?php echo base_url('dashboard_doctor/invoice/item_master/invoice/'.$patientvalue->id)?>"><?php echo $patientvalue->fname." ".$patientvalue->lname ?></td>
            <?php 
                $subscribe = $this->db->get_where("subscribe",array('patient_id' =>$patientvalue->id,"status"=>1 ))->result();
                
                foreach ($subscribe as  $subscribe_value) {      
                    $total = $subscribe_value->amount + $total;
                    $created_date = date("d-m-Y h:i:s",strtotime($subscribe_value->created_date));
                }
            ...
            <td class="text-left"><a href="<?php echo base_url('dashboard_doctor/invoice/item_master/invoice/'.$patientvalue->id)?>"><?php echo $total ?></a></td>
            <td class="text-left"><a href="<?php echo base_url('dashboard_doctor/invoice/item_master/invoice/'.$patientvalue->id)?>"><?php echo $unpaid_total ?></a></td>
			<td class="text-left"><a href="<?php echo base_url('dashboard_doctor/invoice/item_master/invoice/'.$patientvalue->id)?>"><?php echo $Final_total; ?></a></td>
            <!-- <td class="pt-15">
                <div class="btn-group" style="float: right;display: flex;">
                  <a href="<?php //echo base_url('dashboard_doctor/invoice/item_master/invoice/'.$patientvalue->id)?>" class="btn btn-xs icon-box btn-default" style="margin-right:10px;"><i class="fa fa-eye"></i></a>
                   <a href="" class="btn btn-xs icon-box btn-default" style="margin-right:10px;"><i class="fa fa-edit"></i></a>
                  <a href="<?php// echo base_url('dashboard_doctor/invoice/item_master/delete_billing/'.$patientvalue->id) ?>" class="btn btn-xs btn-danger icon-box" style="margin-right:10px;border:1px solid #f00 !important;"><i class="fa fa-trash"></i></a>
                </div>
            </td> -->
          </tr>
        <?php } ?>
...
//patient page search
        function get_patientlist_search() {
            ...
            $("#padd").empty();
            $.ajax({
                url: BASE_URL + "dashboard_doctor/invoice/item_master/get_patient_list",
