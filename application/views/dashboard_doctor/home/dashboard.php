...
<?php   $feature = $this->db->select("*")->from("user")->where("user_id",$this->session->userdata('user_id'))->get()->row();
...
<div class="white-box" style="margin-right: 2%;min-height: 850px;">
  ...
  $user_id = $this->session->userdata('hospital_id');
  $session_email = $this->session->userdata('email');
   // Patient Data
  $PatientActive = $this->db->select("*")->from("patient")->where("created_by",$user_id)->where("status","1")->get()->result();
  $PatientInactive = $this->db->select("*")->from("patient")->where("created_by ",$user_id)->where("status","0")->get()->result();
  // Patient Data

  // Patient patient
  $patientUnread = $this->db->select("*")->from("message")->where("to_user",$session_email)->where("user_type","medical_patient")->where("is_read","0")->get()->result();
  ...
  // Medical Admin count
   $totalSchedule = $this->db->select("*")->from("schedule")->where("doctor_id",$user_id)->get()->result();
   ?>
  <a href="#"> Active Patient - <span><?php echo count($PatientActive); ?></span></a>
  </div>
  <div class="dash-link">
  <a href="#"> Inactive Patient - <span><?php echo count($PatientInactive); ?><span></a>
  </div>

  <?php if(in_array('message',$features_array)){ ?>
      <div class="dash-link">
      <a href="<?php echo base_url().'dashboard_doctor/messages/message/index' ?>"> Patient Unread Message - <span><?php echo count($patientUnread); ?></span></a>
    </div>
    <div class="dash-link">
      <a href="<?php echo base_url().'dashboard_doctor/messages/message/index' ?>"> Medical Provider / Admin Unread Message - <span><?php echo count($medicalAdminUnread); ?><span></a>
    </div>
<?php  }  ?>
...
<?php  }  ?>
  ...
  <div class="dash-link">
  <a href="<?php echo base_url(); ?>dashboard_doctor/patient/patient/create"><i class="fa fa fa-plus"></i>Add Patient</a>
...
<div class="dash-link">
  ...
  <a href="#"> Inactive Patient - <span><?php echo count($PatientInactive); ?><span></a>
   ...
   <a href="<?php echo base_url(); ?>dashboard_doctor/patient/patient"><button type="button" class="btn btn-primary text-center top-72">Patient List</button></a>
        </div>
      </div>
  ...
  <div class="dash-link">
    <?php $actived = $this->db->select("*")->from("user")->where("status","1")->where("user_role","2")->get()->result(); ?>
    <?php $inactived = $this->db->select("*")->from("user")->where("status","0")->where("user_role","2")->get()->result(); ?>
  <a href="<?php echo base_url(); ?>dashboard_doctor/doctor/doctor/create"><i class="fa fa fa-plus"></i>Add <br>  Medical Provider</a>
  <a href="<?php echo base_url(); ?>dashboard_doctor/doctor/doctor/"><i class="fa fa-list-alt"></i>Medical Provider</a>
</div>
<div class="dash-link">
  <a href="#"> Active <br> Medical Provider - <span><?php echo count($actived); ?></span></a>
  <a href="#"> Inactive <br> Medical Provider - <span> <?php echo count($inactived); ?></span></a>
</div>
   <a href="<?php echo base_url(); ?>dashboard_doctor/doctor/doctor"><button type="button" class="btn btn-primary text-center top-6">Medical Provider List</button></a>
        </div>
      </div>
  <?php  }  ?>
  ...
  <p>Electronic health records, includes Vital Signs, e-Prescription, Allergies, Medications & much more. </p>
   ...
   <a href="<?php echo base_url().'dashboard_doctor/health_record/health_report/create'; ?>" class="top-56"><button type="button" class="btn btn-primary text-center">Health Record List</button></a>
        </div>
      </div>
    <?php } ?>
    <?php if(in_array('schedule',$features_array)){ ?>
...
<div class="dash-link">
  ...
  <a href="#"> Total Schedule - <span><?php echo count($totalSchedule); ?></span></a>
   ...
   <a href="<?php echo base_url(); ?>dashboard_doctor/schedule/schedule" class="top-56"><button type="button" class="btn btn-primary text-center">Schedule List</button></a>
        </div>
      </div>
    <?php } ?>
  ...
  <div class="dash-link">
   ...
   <a href="<?php echo base_url(); ?>dashboard_doctor/messages/message/index" class="top-56"><button type="button" class="btn btn-primary text-center">Message List</button></a>
        </div>
      </div>
    <?php } ?>
  ...
  <p>To get all the reporting of Medical Providers, Patients , Health records, Billing in PDF's/Excels. </p>
   ...
   <a href="#" class="top-56"><button type="button" class="btn btn-primary text-center">Reporting List</button></a>
        </div>
      </div>-->
    <?php //} ?>
  ...
  <p>To view dynamic presentable charts of vital signs of all patients. </p>
   ...
   <a href="#" class="top-56"><button type="button" class="btn btn-primary text-center">Charts List</button></a>
        </div>
      </div> -->
    <?php //} ?>
  ...
  <div class="dash-link">
  <a href="<?php echo base_url(); ?>dashboard_doctor/insurance/insurance/create"><i class="fa fa fa-plus"></i>Add Insurance</a>
  <!-- <a href="#"><i class="fa fa-list-alt"></i>Patient Report</a> -->
  </div>
   <a href="<?php echo base_url(); ?>dashboard_doctor/insurance/insurance" class="top-56"><button type="button" class="btn btn-primary text-center">Insurance eligibility List</button></a>
        </div>
      </div>
    <?php } ?>
                   ...
                   <div role="tabpanel">
                       ...
                       <div class="tab-content appointment-tab">
                           <div role="tabpanel" class="tab-pane active" id="patient">
                             <?php echo form_open_multipart('dashboard_doctor/schedule/schedule/create','class="form-inner"') ?>
                             <div class="h-50">
  ...
  $where_in = 'schedule';
                         if($isadmin=='1'){
                           $doctor_l = $this->db->select("*")->from("user")->where("user_role","2")->where('find_in_set("'.$where_in.'", features) <> 0')->get()->result();
                         }else{
                           $doctor_l = $this->db->select("*")->from("user")->where("user_role","2")->where('find_in_set("'.$where_in.'", features) <> 0')->where("user_id",$user_id)->get()->result();
                         }
                             ?>
                           <?php //date_default_timezone_set('Asia/Kolkata');
                         //   $doctor_l = $this->db->select("*")->from("user")->where("user_role","2")->get()->result();  ?>
                               <select class="form-control" required name="doctor_id" id="provider" >
                                 <option value="">Select Provider</option>
                                 <?php foreach ($doctor_l as  $value) {
                                   ?>
                 ...
                 <div class="modal-footer">
                           ...
                           <div role="tabpanel" class="tab-pane" id="block">
                             ...
                             <div class="row">
                               ...
                               <div class="col-12 col-md-5 ml-10 form-group">
                                   <label>BLOCK TIME FOR</label>
                                   <?php //$doctor_ls = $this->db->select("*")->from("user")->where("user_role","2")->get()->result();
                                   $isadmin = $this->session->userdata('isadmin');
          $user_id = $this->session->userdata('user_id');
          $where_in = 'schedule';
                                 if($isadmin=='1'){
                                   $doctor_ls = $this->db->select("*")->from("user")->where("user_role","2")->where('find_in_set("'.$where_in.'", features) <> 0')->get()->result();
                                 }else{
                                   $doctor_ls = $this->db->select("*")->from("user")->where("user_role","2")->where('find_in_set("'.$where_in.'", features) <> 0')->where("user_id",$user_id)->get()->result();
                                 }
                                 ?>
 ...
 <?php if($isadmin=='1'){ ?>
                               ...
                               <label>WHEN</label>
                               <input required type="text" name="bwhens" class="form-control text-field" min="<?php echo date('Y-m-d'); ?>" id="bwhens" placeholder="Group ID">
                             </div>
                  </div>
                  <div class="row">
                           ...
                           <!-- <input list="browsers" name="bduration" id="browser">
                            ...
                            <label>Date</label>
                            <input type="text"  name="repeat_date"  id="repeat_date"  class="form-control text-field"  placeholder="Group ID">
                          </div>
                          </div>
                             <div class="row ">
                             <div class="col-12 form-group ml-10 mr-10">
   ...
   <div class="modal-dialog" style="width: 850px;">
      <div class="modal-content">
        <?php echo form_open('dashboard_doctor/home/update_password','class="form-inner"') ?>
         <?php //echo form_open_multipart('schedule/create','class="form-inner"') ?>
         <?php //echo form_hidden('schedule_id',$schedule->schedule_id); ?>
         <div class="modal-header">
